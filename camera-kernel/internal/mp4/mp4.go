...
func handlerKeyframe(w http.ResponseWriter, r *http.Request) {
	...
	if !captureKeyframe(r.Context(), cons, once) {
		return
	}
...
type keyframeSession interface {
	WriteTo(io.Writer) (int64, error)
	Stop() error
}

func captureKeyframe(ctx context.Context, session keyframeSession, destination io.Writer) bool {
	done := make(chan struct{})
	go func() {
		_, _ = session.WriteTo(destination)
		close(done)
	}()
