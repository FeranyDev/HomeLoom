...
func apiHomeKitSession(w http.ResponseWriter, r *http.Request) {
	...
	if id != "" {
		...
		api.ResponseJSON(w, srv.sessionStatus())
		return
	}
	status := make(map[string]homekit.SessionStatus, len(servers))
	for id, srv := range servers {
		status[id] = srv.sessionStatus()
	}
