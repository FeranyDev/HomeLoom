...
func Init() {
	...
	streams.RedirectFunc("ffmpeg", func(rawURL string) (string, error) {
		if _, err := Version(); err != nil {
			return "", err
		}
		args := parseArgs(rawURL[7:])
		if core.Contains(args.Codecs, "auto") {
			return "", nil // force call streams.HandleFunc("ffmpeg")
		}
		redirect := "exec:" + args.String()
		// Preserve startup timeout so hardware Dial failures fall through the
		// multi-producer chain instead of blocking for the default 30s.
		if i := strings.IndexByte(rawURL, '#'); i > 0 {
			query := streams.ParseQuery(rawURL[i+1:])
			if timeout := query.Get("starttimeout"); timeout != "" {
				redirect += "#starttimeout=" + timeout
			}
		}
		return redirect, nil
	})
	...
	api.HandleFunc("api/frame.jpeg", snapshotHandler(captureSnapshotKeyframe, JPEGWithScaleContext))
...
var defaults = map[string]string{
	...
	"opus/8000":  "-c:a libopus -application:a lowdelay -min_comp 0 -ar:a 8000 -ac:a 1",
	"opus/16000": "-c:a libopus -application:a lowdelay -min_comp 0 -ar:a 16000 -ac:a 1",
	"opus/24000": "-c:a libopus -application:a lowdelay -min_comp 0 -ar:a 24000 -ac:a 1",
...
func parseArgs(s string) *ffmpeg.Args {
	...
	if query != nil {
		...
		// Replace template profile/level when the consumer selects explicit values
		// so we do not emit duplicate -profile:v/-level:v pairs.
		if profile := query.Get("profile"); profile == "baseline" || profile == "main" || profile == "high" {
			replaceOrAddCodec(args, "-profile:v ", "-profile:v "+profile)
		}
		if level := query.Get("level"); level == "3.1" || level == "3.2" || level == "4.0" {
			replaceOrAddCodec(args, "-level:v ", "-level:v "+level)
		}

		if query["bitrate"] != nil {
			// https://trac.ffmpeg.org/wiki/Limiting%20the%20output%20bitrate
			b := query["bitrate"][0]
			args.AddCodec("-b:v " + b + " -bufsize " + multipliedBitrate(b, 5) + " -maxrate " + b)
		}

		if raw := query.Get("framerate"); raw != "" {
			if framerate, err := strconv.Atoi(raw); err == nil && framerate >= 1 && framerate <= 120 {
				keyframeInterval := 4
				if rawInterval := query.Get("keyframe_interval"); rawInterval != "" {
					if interval, intervalErr := strconv.Atoi(rawInterval); intervalErr == nil &&
						interval >= 1 && interval <= 4 {
						keyframeInterval = interval
					}
				}
				// Match Scrypted's HomeKit transcode cadence: the selected
				// frame rate with a four-second default IDR interval. Camera
				// publishers can request a shorter interval when a controller
				// repeatedly asks for a decodable intra frame.
				keyframeFrames := strconv.Itoa(framerate * keyframeInterval)
				args.AddCodec("-g:v " + keyframeFrames +
					" -keyint_min:v " + keyframeFrames +
					" -sc_threshold:v 0 -r:v " + strconv.Itoa(framerate))
			}
		}
		...
		if raw := query.Get("audio_packet_time"); raw != "" {
			if packetTime, err := strconv.Atoi(raw); err == nil &&
				(packetTime == 20 || packetTime == 30 || packetTime == 40 || packetTime == 60) {
				args.AddCodec("-frame_duration:a " + strconv.Itoa(packetTime))
			}
		}
		if bitrate := query.Get("audio_bitrate"); bitrate != "" {
			args.AddCodec("-b:a " + bitrate + " -bufsize:a " + multipliedBitrate(bitrate, 4))
		}
...
func replaceOrAddCodec(args *ffmpeg.Args, prefix, value string) {
	for i, codec := range args.Codecs {
		parts := strings.Fields(codec)
		replaced := false
		for j, part := range parts {
			if part == strings.TrimSpace(prefix) && j+1 < len(parts) {
				parts[j+1] = strings.TrimSpace(strings.TrimPrefix(value, prefix))
				replaced = true
				break
			}
			if strings.HasPrefix(part, strings.TrimSpace(prefix)) {
				parts[j] = strings.TrimSpace(value)
				replaced = true
				break
			}
		}
		if replaced {
			args.Codecs[i] = strings.Join(parts, " ")
			return
		}
	}
	args.AddCodec(value)
...
func doubledBitrate(value string) string {
	return multipliedBitrate(value, 2)
}

func multipliedBitrate(value string, multiplier uint64) string {
	if value == "" {
		return value
	}
	number := value
	suffix := ""
	if last := value[len(value)-1]; last == 'K' || last == 'k' || last == 'M' || last == 'm' {
		number = value[:len(value)-1]
		suffix = value[len(value)-1:]
	}
	parsed, err := strconv.ParseUint(number, 10, 64)
	if err != nil || parsed > ^uint64(0)/multiplier {
		return value
	}
	return strconv.FormatUint(parsed*multiplier, 10) + suffix
}
