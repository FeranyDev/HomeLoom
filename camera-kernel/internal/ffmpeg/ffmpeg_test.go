...
func TestParseArgsHwVaapi(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expect: `ffmpeg -hide_banner -init_hw_device vaapi=va -hwaccel vaapi -hwaccel_device va -filter_hw_device va -hwaccel_output_format vaapi -hwaccel_flags allow_profile_mismatch -re -i /media/bbb.mp4 -c:v mjpeg_vaapi -an -vf "format=vaapi|nv12,hwupload" -f mjpeg -`,
		},
		{
...
func TestDrawText(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expect: `ffmpeg -hide_banner -init_hw_device vaapi=va -hwaccel vaapi -hwaccel_device va -filter_hw_device va -hwaccel_output_format nv12 -hwaccel_flags allow_profile_mismatch -fflags nobuffer -flags low_delay -i http:///example.com -c:v h264_vaapi -g 50 -bf 0 -profile:v high -level:v 4.1 -sei:v 0 -an -vf "scale=640:-1,drawtext=fontsize=12:text='%{localtime\:%Y-%m-%d %X}',hwupload" -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
