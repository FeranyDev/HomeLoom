...
func TestParseArgsDevice(t *testing.T) {
	var videoInput, h265Input, videoAudioInput string
	switch runtime.GOOS {
	case "windows":
		videoInput = `-f dshow -video_size 1920x1080 -i "video=0"`
		h265Input = `-f dshow -framerate 20 -i "video=0"`
		videoAudioInput = `-f dshow -i "video=FaceTime HD Camera:audio=Microphone (High Definition Audio Device)"`
	case "darwin", "ios":
		videoInput = `-f avfoundation -video_size 1920x1080 -i "0:"`
		h265Input = `-f avfoundation -framerate 20 -i "0:"`
		videoAudioInput = `-f avfoundation -i "FaceTime HD Camera:Microphone (High Definition Audio Device)"`
	default:
		videoInput = `-f v4l2 -video_size 1920x1080 -i 0`
		h265Input = `-f v4l2 -framerate 20 -i 0`
		videoAudioInput = `-f v4l2 -i FaceTime HD Camera`
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			expect: `ffmpeg -hide_banner ` + videoInput + ` -c copy -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
		{
			...
			expect: `ffmpeg -hide_banner ` + h265Input + ` -c:v libx265 -g 50 -profile:v main -x265-params level=5.1:high-tier=0 -preset:v superfast -tune:v zerolatency -pix_fmt:v yuv420p -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
		{
			name:   "[DEVICE] video/audio",
			source: "device?video=FaceTime HD Camera&audio=Microphone (High Definition Audio Device)",
			expect: `ffmpeg -hide_banner ` + videoAudioInput + ` -c copy -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func TestParseArgsIpCam(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "[RTSP] video honors the selected output framerate and bitrate",
			source: "rtsp://example.com#video=h264#framerate=15#bitrate=300K",
			expect: `ffmpeg -hide_banner -allowed_media_types video -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp://example.com -c:v libx264 -g 50 -profile:v high -level:v 4.1 -preset:v superfast -tune:v zerolatency -pix_fmt:v yuv420p -b:v 300K -bufsize 1500K -maxrate 300K -g:v 60 -keyint_min:v 60 -sc_threshold:v 0 -r:v 15 -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
		{
			name:   "[RTSP] video supports a one-second HomeKit keyframe interval",
			source: "rtsp://example.com#video=h264#framerate=15#keyframe_interval=1",
			expect: `ffmpeg -hide_banner -allowed_media_types video -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp://example.com -c:v libx264 -g 50 -profile:v high -level:v 4.1 -preset:v superfast -tune:v zerolatency -pix_fmt:v yuv420p -g:v 15 -keyint_min:v 15 -sc_threshold:v 0 -r:v 15 -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
		{
			name:   "[RTSP] invalid output framerate is ignored",
			source: "rtsp://example.com#video=h264#framerate=invalid",
			expect: `ffmpeg -hide_banner -allowed_media_types video -fflags nobuffer -flags low_delay -timeout 5000000 -user_agent go2rtc/ffmpeg -rtsp_flags prefer_tcp -i rtsp://example.com -c:v libx264 -g 50 -profile:v high -level:v 4.1 -preset:v superfast -tune:v zerolatency -pix_fmt:v yuv420p -an -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
		{
...
func TestParseArgsHomeKitSelection(t *testing.T) {
	args := parseArgs("rtsp://example.com#video=h264#audio=opus/24000#profile=main#level=3.1#framerate=15#keyframe_interval=1#bitrate=300K#audio_packet_time=20#audio_bitrate=24K")
	require.Contains(t, args.String(), "-profile:v main -level:v 3.1")
	require.Contains(t, args.String(), "-b:v 300K -bufsize 1500K -maxrate 300K -g:v 15 -keyint_min:v 15 -sc_threshold:v 0 -r:v 15")
	require.Contains(t, args.String(), "-ar:a 24000 -ac:a 1 -frame_duration:a 20 -b:a 24K -bufsize:a 96K")

	args = parseArgs("rtsp://example.com#video=h264#framerate=invalid")
	require.NotContains(t, args.String(), "-r:v")
}

func TestMultipliedBitrate(t *testing.T) {
	require.Equal(t, "598K", doubledBitrate("299K"))
	require.Equal(t, "1495K", multipliedBitrate("299K", 5))
	require.Equal(t, "96K", multipliedBitrate("24K", 4))
	require.Equal(t, "invalid", doubledBitrate("invalid"))
}
...
func TestParseArgsHwVaapi(t *testing.T) {
	...
	switch runtime.GOOS {
	case "windows":
		deviceInput = `-f dshow -video_size 1920x1080 -i "video=0"`
	case "darwin", "ios":
		deviceInput = `-f avfoundation -video_size 1920x1080 -i "0:"`
	default:
		deviceInput = `-f v4l2 -video_size 1920x1080 -i 0`
	}
	...
	tests := []struct {
		...
		},
		{
			...
			expect: `ffmpeg -hide_banner -init_hw_device vaapi -hwaccel_output_format vaapi -hwaccel_flags allow_profile_mismatch -re -i /media/bbb.mp4 -c:v mjpeg_vaapi -an -vf "format=vaapi|nv12,hwupload" -f mjpeg -`,
		},
		{
			...
			expect: `ffmpeg -hide_banner -init_hw_device vaapi -hwaccel_output_format vaapi -hwaccel_flags allow_profile_mismatch ` + deviceInput + ` -c:v hevc_vaapi -g 50 -bf 0 -profile:v main -level:v 5.1 -sei:v 0 -an -vf "format=vaapi|nv12,hwupload" -user_agent ffmpeg/go2rtc -rtsp_transport tcp -f rtsp {output}`,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
