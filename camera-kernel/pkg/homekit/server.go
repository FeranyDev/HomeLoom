...
type ServerAccessory interface {
	...
	SetCharacteristicEvent(conn net.Conn, aid uint8, iid uint64, enabled bool)
	// SetCharacteristic applies a HAP write. When the controller explicitly
	// requests a write response, the accessory may return a value in the same
	// transaction.
	SetCharacteristic(conn net.Conn, aid uint8, iid uint64, value any, writeResponse bool) any
		...
		case hap.PathCharacteristics:
			...
			case "PUT":
				...
				for _, char := range v.Value {
					if char.Event != nil {
						server.SetCharacteristicEvent(conn, char.AID, char.IID, *char.Event)
					}
					if len(char.Value) == 0 {
						continue
					}
					var decoded any
					if err := json.Unmarshal(char.Value, &decoded); err != nil {
						return nil, err
					}
					writeResponse := char.Response != nil && *char.Response
					value := server.SetCharacteristic(conn, char.AID, char.IID, decoded, writeResponse)
					if writeResponse && value != nil {
						status := 0
						response.Value = append(response.Value, hap.JSONCharacter{
							AID:    char.AID,
							IID:    char.IID,
							Status: status,
							Value:  value,
				...
				return &http.Response{
					...
					ProtoMinor: 1,
					Header: http.Header{
						"Content-Type":   []string{hap.MimeJSON},
