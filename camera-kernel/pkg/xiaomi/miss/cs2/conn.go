...
func Dial(host, transport string) (*Conn, error) {
	...
	_, isTCP := conn.(*tcpConn)
		...
		channels: [4]*dataChannel{
			// Media channel (2): larger pop buffer so a temporary FFmpeg/HomeKit
			// stall does not kill the CS2 worker with "pop buffer is full".
			newDataChannel(0, 10), nil, newDataChannel(250, 512), nil,
		},
	}
	go c.worker()
	return c, nil
...
func (c *dataChannel) Push(b []byte) error {
	...
	for len(c.waitData) > 4 {
		...
		case c.popBuf <- frame:
		default:
			// Drop the oldest queued frame and keep the newest so the media
			// worker survives brief consumer stalls instead of tearing down
			// the whole MISS session (which caused repeated keyframe loss).
			select {
			case <-c.popBuf:
			default:
			}
