...
func RTPDepay(codec *core.Codec, handler core.HandlerFunc) core.HandlerFunc {
	...
	// A HomeKit consumer can attach while the producer is in the middle of a
	// fragmented NAL unit. Feeding orphan FU-A tails to pion synthesizes an
	// incomplete NAL header; an orphan IDR tail can then look like a keyframe
