...
const (
	...
	// Placeholder; Payload recomputes max NRI from aggregated NALUs.
	outputStapAHeader = 0x18
)
...
func (p *Payloader) Payload(mtu uint16, payload []byte) [][]byte {
	...
	EmitNalus(payload, p.IsAVC, func(nalu []byte) {
		...
		case spsNALUType, ppsNALUType:
			...
			// Raise STAP-A NRI to the max nal_ref_idc of aggregated units (RFC 6184).
			if nri := nalu[0] & naluRefIdcBitmask; nri > (p.stapANalu[0] & naluRefIdcBitmask) {
				p.stapANalu[0] = nri | stapaNALUType
			}
			return
		}
