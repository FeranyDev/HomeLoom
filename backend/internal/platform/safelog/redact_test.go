...
func TestCoreRedactsKeysAndEmbeddedAssignments(t *testing.T) {
	...
	logger.Error("cloud failed", zap.String("ssecurity", "miot-security-value"), zap.String("passToken", "camera-pass-token-value"), zap.String("deviceKey", "sonoff-device-key"))

	text := output.String()
	for _, secret := range []string{"plain-secret", "url-secret", "error-secret", "miot-security-value", "camera-pass-token-value", "sonoff-device-key"} {
