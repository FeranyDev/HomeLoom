...
func (s *Store) SaveMappingBinding(ctx context.Context, item mapping.Binding) error {
	...
	row := mappingBindingRow{ID: item.ID, Stage: string(stage), ProfileID: item.ProfileID, ProviderID: item.ProviderID, DeviceID: item.DeviceID, EndpointID: item.EndpointID, CapabilityID: item.CapabilityID, PropertyID: item.PropertyID, DeviceType: string(item.DeviceType), ConsumerDeviceType: string(item.ConsumerDeviceType), ModelEndpointID: model.EndpointID, ModelCapabilityID: model.CapabilityID, ModelPropertyID: model.PropertyID, ConsumerID: item.ConsumerID, TargetID: item.TargetID, ConsumerDeviceID: item.ConsumerDeviceID, ConsumerProperty: item.ConsumerProperty, Enabled: item.Enabled, ReadbackEnabled: item.ReadbackEnabled, ReadbackDelaysJSON: string(readbackDelays), CreatedAt: now, UpdatedAt: now}
	err = s.orm.WithContext(ctx).Clauses(clause.OnConflict{Columns: []clause.Column{{Name: "id"}}, DoUpdates: clause.AssignmentColumns([]string{"stage", "profile_id", "provider_id", "device_id", "endpoint_id", "capability_id", "property_id", "device_type", "consumer_device_type", "model_endpoint_id", "model_capability_id", "model_property_id", "consumer_id", "target_id", "consumer_device_id", "consumer_property", "enabled", "readback_enabled", "readback_delays_json", "updated_at"})}).Create(&row).Error
	if err != nil {
