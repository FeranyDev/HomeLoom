...
状态：实现中（阶段 A 主链完成；阶段 B/C/D 部分完成）  
...
- [ ] 为瞬时 Event 增加独立应用层/Consumer 投递通道；当前只解析并计数，不伪装成属性。
...
- [ ] 增加基于失败次数、断线宽限和 stale 的属性级调度；
- [x] 根据通知能力和补偿间隔计算动态 TTL；可推送属性使用较长宽限，不可推送属性至少覆盖两轮补偿。
- [ ] 接入统一模型显式 `staleAfterSeconds` 覆盖与连续失败退避。
...
- [x] 补偿间隔文案不再描述为固定轮询；
- [ ] 完成两条 MQTT 通道的断线原因、最近成功时间和全部诊断指标；
