...
- [x] Provider 回退策略（Logical Device 按 Provider 优先级路由；仅明确不可用时回退，命令还要求幂等）；
...
- [x] Provider Device ID 到内部 Device ID 绑定表；
- [x] Logical Device ID 表；
- [x] Device Endpoint/Capability 身份表；
- [x] HomeKit Accessory UUID 映射表；
- [x] AID 持久化表；
- [x] IID 持久化表；
- [x] 设备改名身份不变测试；
- [x] Capability 顺序变化 IID 不变测试；
- [x] 设备离线、禁用或 Provider 删除均不删除稳定身份；
- [~] 显式删除保留期（Logical Device 显式解绑保留 30 天并提供显式过期清理；通用设备删除策略仍待产品定义）。
...
- [x] 配置导出脱敏；
- [x] 诊断包脱敏；
- [x] 主密钥版本与轮换：`enc:v2:<key-version>:` AES-GCM 密文绑定版本和 scope；兼容 `enc:v1` 旧密文，keyring 支持旧 key 读取、新 key 写入；
- [x] 管理员确认 `ROTATE` 后批量事务重加密 Target、Matter、Provider 与媒体凭据；中断后可安全恢复，不返回任何 key 或明文；
- [x] 轮换保留旧 key 以解密历史备份，当前不提供自动销毁/裁剪旧 key（须先定义备份保留策略）；
...
- [x] Capability Profile（可用于精确属性运行时绑定，并按可逆数值单位差异自动生成内置转换规则）；
...
- [x] 单位转换；
- [x] 默认值；
- [x] 缺失属性（仅在显式绑定的来源路径缺失时使用 Profile 默认值；实际来源值始终优先）；
- [ ] 多属性组合；
- [x] 写入反向转换；
- [x] 转换错误解释。
...
- [x] HAP Consumer 完整属性目录、运行时投影和统一模型当前值的逐值结果预览；
- [x] 前端 Profile 管理；
...
- [x] 状态来源和质量展示；
- [x] 命令历史页面；
- [x] 实时日志（`runtime_logs` 已统一主进程与子进程日志，并通过 SSE 实时输出）；
- [x] Mapping 预览；
- [x] Profile 管理；
...
- [x] Token 获取、加密持久化、70% 有效期自动刷新和指数退避已实现；管理员可经 CSRF 和精确确认主动注销：始终禁用 Provider、断开会话并清除本地 Token/证书/账号会话，保留设备映射供重新授权；中枢远端注销只会调用管理员显式配置的 HTTPS OAuth `revocationEndpoint`，不猜测小米 URL；第三方兼容云暂无可信远端注销契约，仅执行本地安全注销；
