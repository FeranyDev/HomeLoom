...
真实 driver 当前覆盖 25 类官方标准 Device Type：开关、插座、扩展彩灯、温度、湿度、接触、活动、占用、网络设备、窗帘、风扇、恒温器、门锁、照度、气压、漏水、烟雾、一氧化碳、空气质量、阀门、水泵、空气净化器、扬声器、电视、电力计量器。网络设备在统一模型中保留为 `network-device` 和“电源状态”；Matter 没有专用 LAN 电源 Device Type，因此仅在目标边界使用官方 On/Off Plug-in Unit Endpoint。其 On 操作会映射到 `switch/power=true` 并发送 Wake-on-LAN，TCP 探测再确认实际开机状态。电视使用 Basic Video Player Endpoint，并映射 Media Playback 的播放控制和 Keypad Input 的遥控按键（包括音量加/减）。Matter Basic Video Player 的标准 AudioOutput 不提供绝对音量属性，因此 Matter 侧不伪造音量滑杆；绝对音量仍由统一模型和 HomeKit Television Volume 提供。电力计量器使用 Electrical Meter Endpoint：有功/无功/视在功率、电压、电流、频率、功率因数和累计进口电量由运行时转换为 Matter 的 mW/mV/mA/mHz/mWh 整数属性，并附带标准 Measurement Accuracy 声明。测试会直接读取 Go Matter Catalog，要求每个公开类型和 Cluster Attribute/Command 路径都能由官方 matter.js Endpoint 构造，避免管理端暴露运行时无法重放的类型。
...
日常 CI 覆盖 IPC 合约、背压、断线恢复、双实例隔离、当前已开放的 25 种标准 Matter Device Type、100 Endpoint Go 快照、Endpoint 稳定身份、数据库恢复和前端操作。以下项目需要维护者在真实局域网执行：
