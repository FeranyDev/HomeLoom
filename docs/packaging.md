...
启用 Matter 后仍是两个持续运行的进程：Go 主服务和 Matter runtime 子进程。启用 MCP/AI 时，Core 会额外拉起并监管同一容器/主机中的 `homeloom-mcp-agent` 子程序；它持有 AI API Key，Core 不将 Key 写入数据库、Provider 配置或审计详情。打包脚本同时保留两种 Matter 运行方式：默认 `js` 模式使用 Node.js + `matter-runtime/dist/src/cli.js`，`sea` 模式把后者制作成单文件可执行程序，因此 SEA 部署设备不需要安装 Node.js、npm 或携带 `node_modules`。这仍不是严格的单进程单二进制；子进程崩溃隔离和按 Target 重启能力保持不变。
...
同一目录还会生成 `backend/bin/homeloom-mcp-agent`。它只在 macOS/Linux 上配合 Core 的 Unix Socket 使用；运行参数与令牌文件要求见 [MCP 与 AI Agent](mcp-ai-agent.md)。
...
- 每个目标目录中的 `homeloom-mcp-agent`（Windows 为 `.exe`，但 Unix Socket MCP 部署不支持 Windows）；
- `SHA256SUMS`。
...
- Core 与独立 MCP Agent 的编译检查；MCP Agent 与 Core 的 Unix Socket 协议、批准门和 AI Responses API 请求单元测试。
