...
重新读取 `/accessories` 而无需删除配对。HomeKit Camera 当前固定 IPv4：HAP TCP 强制使用
`tcp4`，mDNS 仅监听 IPv4 并发布 A 记录，SRTP 会话也固定使用 `udp4`；IPv6
SetupEndpoints offer 会被明确拒绝。H.264 RTP Consumer 中途
...
项目当前固定的 matter.js `0.17.7` 已包含 Matter 1.5 Camera、Camera AV Stream
