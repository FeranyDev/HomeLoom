...
function Dashboard({ username, onLogout }: { username: string, onLogout: () => Promise<void> }) {
  ...
  async function handlePowerChange(device: Device, value: boolean) {
	...
	const pageCopy = page === 'devices' ? { title: <>把家的状态<br />织在一起。</>, intro: '设备状态驻留内存，映射从每台设备独立进入配置。', eyebrow: 'DEVICES', section: '设备中心' } : page === 'xiaomi' ? { title: <>让米家设备，<br />回到本地。</>, intro: '从账号授权、中枢发现到设备映射，统一管理米家局域网接入。', eyebrow: 'XIAOMI HOME', section: '米家' } : page === 'providers' ? { title: <>数据源，随时<br />接入或离开。</>, intro: 'Provider 配置存储于 SQLite，并可在线启停和替换。', eyebrow: 'PROVIDERS', section: 'Provider 管理' } : page === 'targets' ? { title: <>一座桥，或<br />很多座桥。</>, intro: '按设备或平台拆分桥实例。每座桥拥有独立身份、端口、配对资料和二维码。', eyebrow: 'TARGETS', section: '桥接中心' } : page === 'mapping' ? { title: <>让转换规则，<br />成为积木。</>, intro: '维护可复用的统一模型自定义属性和转换 Profile；设备路由从对应设备进入。', eyebrow: 'MODEL TOOLS', section: '模型与转换工具' } : { title: <>看见系统的<br />每一次呼吸。</>, intro: '观察事件队列、设备连接和命令生命周期。', eyebrow: 'SYSTEM', section: '系统诊断' }
    ...
    <main id="main-content" tabIndex={-1}>
	  <nav className="top-nav" aria-label="主要导航">
	    ...
	    <div>
	      ...
	      <button aria-current={page === 'mapping' ? 'page' : undefined} className={page === 'mapping' ? 'is-active' : ''} onClick={() => setPage('mapping')}>模型工具</button>
      ...
      <header className="hero">
        ...
        <div className="summary">
		  <span>{summary}</span><small>{page === 'devices' ? '在线设备' : page === 'xiaomi' ? '米家在线设备' : page === 'providers' ? '运行中 Provider' : page === 'targets' ? '运行中的桥' : page === 'mapping' ? '映射工具' : '已处理事件'}</small>
        </div>
      </header>
