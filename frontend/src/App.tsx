  ...
  return (<>
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      <header className="hero">
		...
		<div className="summary" aria-label={`${summary} ${page === 'devices' ? '在线设备' : page === 'providers' ? '运行中设备来源' : page === 'targets' ? '运行中目标' : page === 'mapping' ? '统一设备模型' : '已处理事件'}`}>
		  <span>{summary}</span><small>{page === 'devices' ? '在线设备' : page === 'providers' ? '运行中设备来源' : page === 'targets' ? '运行中目标' : page === 'mapping' ? '统一设备模型' : '已处理事件'}</small>
        </div>
      </header>
