...
const SonoffDeviceManager = lazy(() => import('./components/SonoffDeviceManager').then((module) => ({ default: module.SonoffDeviceManager })))
const TuyaDeviceManager = lazy(() => import('./components/TuyaDeviceManager').then((module) => ({ default: module.TuyaDeviceManager })))
...
function Dashboard({ username, onLogout }: { username: string, onLogout: () => Promise<void> }) {
  ...
  async function handlePowerChange(device: Device, value: boolean) {
	...
	const pageCopy = page === 'devices' ? { title: '设备', help: '查看设备状态、属性和映射。', eyebrow: 'DEVICES', section: '设备中心' } : page === 'providers' ? { title: '设备来源', help: '连接并管理设备来源。', eyebrow: 'PROVIDERS', section: '设备来源' } : page === 'targets' ? { title: '桥接', help: '将设备发布到 HomeKit 或 Matter。', eyebrow: 'TARGETS', section: '桥接中心' } : page === 'mapping' ? { title: '统一模型', help: '定义设备类型、属性和转换规则。', eyebrow: 'UNIFIED MODELS', section: '统一模型' } : page === 'ai' ? { title: 'AI', help: '配置 AI 服务和设备访问范围。', eyebrow: 'AI CONTROL', section: 'AI' } : { title: '系统', help: '查看连接、队列和命令状态。', eyebrow: 'SYSTEM', section: '系统诊断' }
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      <header className="hero">
        <div>
		  ...
		  <h1><HelpTooltip label={`${pageCopy.title}说明`} content={pageCopy.help}>{pageCopy.title}</HelpTooltip></h1>
        </div>
      ...
      {loading ? (
		...
		{page === 'devices' ? <section className="device-grid" aria-label="设备列表">
		  ...
		  <div className="config-note">
		    <span>发布方式</span>
		    <strong>{targetSection === 'homekit-camera' ? '独立 HomeKit 摄像头' : targetSection === 'matter-camera' ? '实验性 Matter 摄像头' : targetSection === 'other-camera' ? '暂未支持' : 'HomeKit / Matter 桥接'}</strong>
		    <HelpTooltip label="目标类型说明" content={targetSection === 'homekit-camera' ? '每个目标只发布一台摄像头，并使用独立的配对身份。' : targetSection === 'matter-camera' ? '每个目标只发布一台摄像头。Controller 兼容性仍在验证，Apple Home 不保证支持。' : targetSection === 'other-camera' ? '该分类预留给后续的媒体协议适配器。' : '普通桥接仅发布非摄像头设备；摄像头请从专属分页发布。'} />
		  </div>
