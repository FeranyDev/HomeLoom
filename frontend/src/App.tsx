...
import { executeDeviceCommand, listDevices, setDevicePower, setDeviceProperty, simulateDevice, subscribeDevices } from './api/devices'
...
export function App() {
  ...
  async function handlePowerChange(device: Device, value: boolean) {
	...
	async function handleCommandExecute(device: Device, endpointId: string, capabilityId: string, commandId: string, parameters: Record<string, PropertyValue>) { try { const updated = await executeDeviceCommand(device.id, endpointId, capabilityId, commandId, parameters); setDevices((current) => current.map((item) => item.id === updated.id ? updated : item)); const [diagnosticData, commandData] = await Promise.all([getDiagnostics(), listCommands()]); setDiagnostics(diagnosticData); setCommands(commandData); setError(null); notify('success', `${device.name}.${commandId} 执行成功`) } catch (cause) { notify('error', cause instanceof Error ? cause.message : '命令执行失败'); throw cause } }
