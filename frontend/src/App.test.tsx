...
describe('App integration', () => {
  ...
  it('navigates management pages and returns to login after logout', async () => {
	...
	expect(await screen.findByRole('heading', { name: '设备来源', level: 2 })).toBeInTheDocument()
	...
	expect(await screen.findByText('统一接入设备来源')).toBeInTheDocument()
  ...
  it('reads the unified model summary count from the model catalog', async () => {
	...
	it('separates HomeKit Camera publication from ordinary bridge targets', async () => {
		...
		expect(screen.getByText('独立 HomeKit 摄像头')).toBeInTheDocument()
		...
		expect(screen.getByText('实验性 Matter 摄像头')).toBeInTheDocument()
		await user.hover(screen.getByRole('button', { name: '目标类型说明' }))
