...
export function requiredXiaomiProperties(type: string) {
	...
	case 'air-conditioner': return [property('air-conditioner', 'active', '启用', 'bool', 1, true), property('air-conditioner', 'current-state', '当前工作状态', 'enum', 2, false, { off: 0, idle: 1, cooling: 2, heating: 3, drying: 4, 'fan-only': 5 }), property('air-conditioner', 'target-mode', '运行模式', 'enum', 3, true, { off: 0, auto: 1, cool: 2, heat: 3, dry: 4, fan: 5 }), property('temperature', 'current-temperature', '当前温度', 'number', 4, false), property('temperature', 'target-temperature', '目标温度', 'number', 5, true)]
