...
describe('API client', () => {
  ...
  it('preserves status and field errors', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(JSON.stringify({ message: 'invalid config', fields: { name: 'required' } }), { status: 400, headers: { 'Content-Type': 'application/json' } })))
    ...
    expect(error.status).toBe(400); expect(error.message).toBe('invalid config'); expect(error.fields).toEqual({ name: 'required' })
  })
})
