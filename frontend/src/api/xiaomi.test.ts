...
describe('Xiaomi API', () => {
	it('starts and completes OAuth and discovers gateways', async () => {
		const fetchMock = vi.fn()
			...
			.mockResolvedValueOnce(new Response(JSON.stringify({ data: [{ did: '2', name: 'Wi-Fi AC', model: 'xiaomi.aircondition.v1' }] }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
		...
		await expect(discoverXiaomiDevices('xiaomi-miot-cloud-main', 'xiaomi-miot-cloud')).resolves.toEqual([expect.objectContaining({ did: '2', name: 'Wi-Fi AC' })])
