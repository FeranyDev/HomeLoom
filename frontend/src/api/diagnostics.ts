...
export function subscribeAuditEvents(onEvent: (event: AuditEvent) => void): () => void {
	const source = new EventSource('/api/v1/events/audit')
	source.addEventListener('audit', (event) => { try { onEvent(JSON.parse(event.data) as AuditEvent) } catch { /* ignore malformed events */ } })
	return () => source.close()
}
