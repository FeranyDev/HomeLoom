  ...
  return <section className="binding-manager mapping-graph">
    ...
    <div className="mapping-lanes">
      ...
      <section className={`mapping-lane ${stage === 'consumer' ? '' : 'is-context'}`}><header><span>消费端（CONSUMERS）</span><strong>{consumerCatalogs.map((item) => item.name).join(' / ') || consumerId || '目标完整属性'}</strong><small>{consumers.length} 个属性</small></header>
        {stage === 'consumer' ? consumers.length > 0 ? <div className="mapping-node-list">{matterConsumerGroups.map((group) => <section className="matter-cluster-group" key={group.cluster}><header><span>Cluster</span><strong>{matterClusterLabel(group.cluster)}</strong><small>Cluster → Attribute / Command</small></header>{group.items.map((item) => { const key = `${item.consumer.id}/${item.property.id}`; const member = matterMember(item.property); return <button key={key} style={selectablePropertyCardStyle} className={key === consumerKey ? 'is-selected' : ''} onClick={() => setConsumerKey(key)}><span>Matter · {member.kind === 'command' ? 'Command（命令）' : 'Attribute（属性）'}</span><strong>{matterConsumerPathLabel(member.cluster, member.member, member.kind)}</strong><code>{member.cluster} → {member.kind === 'command' ? 'Command' : 'Attribute'} → {member.member}</code><small>{valueTypeLabel(item.property.type)} · {parameterLevelLabel(item.property.level)} · {permissionLabel(item.property.readable, item.property.writable, item.property.notifiable)}</small>{item.property.type === 'enum' && item.property.enum?.length ? <small className="enum-domain-inline">消费端枚举：{item.property.enum.join(' / ')}</small> : null}</button> })}</section>)}{otherConsumers.map((item) => { const key = `${item.consumer.id}/${item.property.id}`; return <button key={key} style={selectablePropertyCardStyle} className={key === consumerKey ? 'is-selected' : ''} onClick={() => setConsumerKey(key)}><span>{item.consumer.name}（{item.consumer.id}）</span><strong>{consumerPropertyLabel(item.property.id)}</strong><code>{item.property.id}</code><small>{valueTypeLabel(item.property.type)} · {parameterLevelLabel(item.property.level)} · {permissionLabel(item.property.readable, item.property.writable, item.property.notifiable)}</small>{item.property.type === 'enum' && item.property.enum?.length ? <small className="enum-domain-inline">消费端枚举：{item.property.enum.join(' / ')}</small> : null}</button> })}</div> : <div className="mapping-context"><b>暂无消费端属性目录</b><p>目标适配器 {consumerId ?? '未指定'} 尚未发布该设备模型的属性，不能回退使用其他消费者的属性。</p></div> : <div className="mapping-context"><b>消费端边界（Consumer）</b><p>属性目录由具体目标适配器发布，统一模型层不预设 HomeKit、Matter 或其他协议字段。</p></div>}
      </section>
    </div>
