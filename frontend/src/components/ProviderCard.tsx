...
export function ProviderCard({ provider, devices, onEdit, onDelete, onRestart, onSimulate, onManageDevices, onTest }: {
	...
	const setupReady = provider.type === 'xiaomi' ? authorized : provider.type === 'mqtt' ? brokerReady : configuredDevices.length > 0
	const setupLabel = provider.type === 'xiaomi' ? (authorized ? '已授权' : '待授权') : provider.type === 'mqtt' ? (brokerReady ? '已配置' : '待配置') : `${configuredDevices.length} 台`
	const setupDetail = provider.type === 'xiaomi' ? `${String(oauth.region || 'cn').toUpperCase()} · UID ${String(oauth.uid || '—')}` : provider.type === 'mqtt' ? String(config.brokerUrl || '尚未设置 Broker') : '数据库期望设备'
	const connectionReady = provider.status === 'running'
	const connectionLabel = provider.type === 'xiaomi' ? (certificateReady ? '证书就绪' : '待申请证书') : connectionReady ? '已连接' : provider.status
	const connectionDetail = provider.type === 'xiaomi' ? `${String(config.host || '未选择中枢')}:${Number(config.port || 8883)}` : provider.type === 'mqtt' ? `${String(config.topicPrefix || 'homeloom')} · QoS ${Number(config.qos ?? 1)}` : '进程内异步事件源'
	...
	return <article className="provider-card provider-runtime-card">
		<header>
			...
			<div className="provider-card__actions"><button onClick={() => onEdit(provider)}>{provider.type === 'xiaomi' ? '账号与中枢' : '配置'}</button>{provider.type === 'xiaomi' && onManageDevices && <button disabled={!connectionReady} title={connectionReady ? '通过当前 MQTT 连接读取和配置子设备' : '请先连接中枢 MQTT'} onClick={() => onManageDevices(provider)}>管理子设备</button>}{onTest && <button disabled={busy !== null || (provider.type === 'xiaomi' && !certificateReady)} onClick={() => void run('test')}>{busy === 'test' ? '测试中…' : '测试连接'}</button>}{provider.enabled && <button disabled={busy !== null} onClick={() => void run('restart')}>{busy === 'restart' ? '重连中…' : '重新连接'}</button>}<button className="is-danger" onClick={() => onDelete(provider)}>删除</button></div>
		</header>
		<div className="provider-status-grid">
			<div><span>{provider.type === 'xiaomi' ? '账号与凭据' : provider.type === 'mqtt' ? 'Broker 配置' : '设备配置'}</span><strong className={setupReady ? 'is-ready' : ''}>{setupLabel}</strong><small>{setupDetail}</small></div>
