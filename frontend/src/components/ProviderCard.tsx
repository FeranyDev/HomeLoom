...
export function ProviderCard({ provider, devices, onEdit, onDelete, onRestart, onSimulate, onManageDevices, onTest, onAuthChallengeComplete }: {
	...
	const expectedDeviceCount = provider.type === 'virtual' && !Array.isArray(config.devices) ? devices.length : provider.type === 'gree' && configuredDevices.length === 0 && greeHost ? 1 : configuredDevices.length
	...
	const providerEditLabel = authRequired ? '继续短信验证' : provider.type === 'camera' || provider.type === 'virtual' ? 'Provider 配置' : provider.type === 'xiaomi' ? '账号与中枢' : provider.type === 'xiaomi-miot-cloud' ? '云账号配置' : provider.type === 'gree' ? '设备与连接' : provider.type === 'mqtt' ? (mqttMode === 'server' ? '监听配置' : 'Broker 配置') : '配置'
	...
	return <article className="provider-card provider-runtime-card">
		...
		<div className="provider-status-grid">
			...
			<div><span>期望设备</span><strong>{expectedDeviceCount}</strong><small>{provider.type === 'mqtt' ? '数据库设备路由 · 严格白名单' : provider.type === 'gree' ? '格力设备管理器配置' : '数据库配置'}</small></div>
