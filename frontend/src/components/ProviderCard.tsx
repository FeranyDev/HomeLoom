...
export function ProviderCard({ provider, devices, onEdit, onDelete, onRestart, onSimulate, onManageDevices, onDeviceLocation, onRevokeCredentials, onTest, onAuthChallengeComplete }: {
	...
	return <article className="provider-card provider-runtime-card">
		<header>
			...
			<div className="provider-card__actions"><button onClick={() => onEdit(provider)}>{providerEditLabel}</button>{managedDeviceSource && onManageDevices && <span className="help-action"><button disabled={!connectionReady} onClick={() => onManageDevices(provider)}>{provider.type === 'camera' ? '管理摄像头' : provider.type === 'virtual' ? '管理虚拟设备' : provider.type === 'gree' ? '管理格力设备' : provider.type === 'network' ? '管理网络设备' : '管理设备'}</button>{!connectionReady && <HelpTooltip content="连接设备来源后才能管理设备。" label="管理设备说明" />}</span>}{onTest && <button disabled={busy !== null || (provider.type === 'xiaomi' && !certificateReady)} onClick={() => void run('test')}>{busy === 'test' ? '测试中…' : provider.type === 'mqtt' && mqttMode === 'server' ? '测试监听' : '测试连接'}</button>}{provider.enabled && <button disabled={busy !== null} onClick={() => void run('restart')}>{busy === 'restart' ? (provider.type === 'mqtt' && mqttMode === 'server' ? '重启监听中…' : '重连中…') : provider.type === 'mqtt' && mqttMode === 'server' ? '重启监听' : '重新连接'}</button>}{(provider.type === 'xiaomi' || provider.type === 'xiaomi-miot-cloud') && onRevokeCredentials && !credentialsRevoked && <button className="is-danger" disabled={busy !== null} onClick={() => void run('revoke')}>{busy === 'revoke' ? '注销中…' : '注销凭据'}</button>}<button className="is-danger" onClick={() => onDelete(provider)}>删除</button></div>
		</header>
		...
		<div className="provider-runtime"><span><HelpTooltip content="所有计数均从本次运行开始累计；最终失败表示本地和回退路径都未成功。" label="运行指标说明">运行指标</HelpTooltip></span><span>能力 <b>{features.length}</b></span>{provider.type === 'mqtt' ? <><span>消息 <b>{provider.metrics?.messagesReceived ?? 0}</b></span><span>无效 <b>{provider.metrics?.messagesInvalid ?? 0}</b></span><span>丢弃 <b>{provider.metrics?.messagesDropped ?? 0}</b></span><span>命令 <b>{provider.metrics?.commandsPublished ?? 0}</b></span></> : <><span>累计请求 <b>{provider.metrics?.requests ?? 0}</b></span><span>状态事件 <b>{provider.metrics?.events ?? 0}</b></span><span>最终失败 <b>{provider.metrics?.errors ?? 0}</b></span>{provider.type === 'xiaomi' && <><span>本地请求 <b>{provider.metrics?.localRequests ?? 0}</b></span><span>本地失败 <b>{provider.metrics?.localFailures ?? 0}</b></span><span>自动转云 <b>{provider.metrics?.cloudFallbacks ?? 0}</b></span><span>云 HTTP 请求 <b>{provider.metrics?.cloudRequests ?? 0}</b></span><span>官方云 MQTT <b>{cloudMqttLabel}</b></span><span>云 MQTT 推送 <b>{provider.metrics?.cloudMqttMessagesReceived ?? 0}</b></span></>}</>}</div>
		...
		<div className="provider-device-list"><div className="command-heading"><h3><HelpTooltip content="完成发现或设备配置后，设备会显示在这里。" label="已发布设备说明">已发布设备</HelpTooltip></h3><span>{devices.length} 台</span></div>{devices.length === 0 ? <p>暂无设备</p> : devices.map((item) => <div key={item.id}><span className={`status-dot is-${item.availability}`} /><div className="provider-device-name"><strong>{item.name}</strong>{onDeviceLocation && <button type="button" onClick={() => onDeviceLocation(item)}>{item.locationMode === 'custom' ? `${item.homeName}${item.roomName ? ` / ${item.roomName}` : ''}` : '设置位置'}</button>}</div><code>{item.id}</code><small className="provider-device-runtime"><span>{item.type === 'network-device' ? '网络设备 · 电源状态' : item.type} · seq {item.sequence ?? '—'}</span>{(provider.type === 'xiaomi' || provider.type === 'xiaomi-miot-cloud') && item.runtimeMode && <i className={`device-runtime-mode is-${item.runtimeMode}`}>{runtimeModeLabel(item.runtimeMode, item.stateTransport)}</i>}</small></div>)}</div>
		{provider.type === 'virtual' && devices.length > 0 && <div className="simulation-panel"><div><strong><HelpTooltip content="状态只保存在内存中，重启后会按配置重建。" label="运行时模拟说明">运行时模拟</HelpTooltip></strong></div>{devices.map((item) => {
