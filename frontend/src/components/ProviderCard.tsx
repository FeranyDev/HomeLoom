...
export function ProviderCard({ provider, devices, onEdit, onDelete, onRestart, onSimulate, onManageDevices, onTest }: {
	...
	const expectedDeviceCount = provider.type === 'virtual' && !Array.isArray(config.devices) ? devices.length : configuredDevices.length
	...
	const setupReady = provider.type === 'camera' || provider.type === 'virtual' ? provider.enabled : provider.type === 'xiaomi' ? authorized : provider.type === 'xiaomi-miot-cloud' ? cloudSessionReady : provider.type === 'mqtt' ? brokerReady : configuredDevices.length > 0
	const setupLabel = provider.type === 'camera' ? `${configuredDevices.length} 台子设备` : provider.type === 'virtual' ? `${expectedDeviceCount} 台子设备` : provider.type === 'xiaomi' ? (authorized ? '已授权' : '待授权') : provider.type === 'xiaomi-miot-cloud' ? (cloudSessionReady ? '账号就绪' : '待登录') : provider.type === 'mqtt' ? (brokerReady ? (mqttMode === 'server' ? '监听就绪' : 'Broker 就绪') : '待配置') : `${configuredDevices.length} 台`
	...
	const managedDeviceSource = provider.type === 'virtual' || provider.type === 'camera' || provider.type === 'mqtt' || provider.type === 'xiaomi' || provider.type === 'xiaomi-miot-cloud'
	...
	return <article className="provider-card provider-runtime-card">
		...
		<div className="provider-status-grid">
			...
			<div><span>期望设备</span><strong>{expectedDeviceCount}</strong><small>{provider.type === 'mqtt' ? '数据库设备路由 · 严格白名单' : '数据库配置'}</small></div>
