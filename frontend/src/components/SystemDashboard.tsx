...
function RuntimeSettingsCard({ settings, onSave }: { settings: RuntimeSettings | null; onSave: (settings: RuntimeSettings) => Promise<void> }) {
  ...
  return <div className="queue-card settings-card"><div><span>命令运行时设置</span><strong>{settings?.commandTimeoutSeconds ?? seconds}s / {settings?.commandHistoryLimit ?? historyLimit} 条</strong></div><label>确认超时秒数<input aria-label="命令确认超时秒数" type="number" min="1" max="300" value={seconds} onChange={(event) => setSeconds(Number(event.target.value))} /></label><label>历史保留上限<input aria-label="命令历史保留上限" type="number" min="100" max="10000" value={historyLimit} onChange={(event) => setHistoryLimit(Number(event.target.value))} /></label><button disabled={saving || !settings} onClick={() => void save()}>{saving ? '保存中…' : '保存并实时应用'}</button>{error && <small className="field-error" role="alert">{error}</small>}<small>存储于 PostgreSQL。超时仅影响之后创建的命令；降低历史上限会立即清理最旧的终态记录，不删除执行中的命令。</small></div>
}
...
function DatabaseMaintenanceCard() {
	...
	async function backup() {
		...
		try {
			...
			setMessage('完整 PostgreSQL 逻辑快照已生成。HomeKit 控制器配对文件不在此压缩包内。')
...
export function SystemDashboard({ diagnostics, commands, auditEvents = [], settings, onSettingsSave }: { diagnostics: Diagnostics | null; commands: DeviceCommand[]; auditEvents?: AuditEvent[]; settings: RuntimeSettings | null; onSettingsSave: (settings: RuntimeSettings) => Promise<void> }) {
  ...
  const metrics = [
	...
	['PostgreSQL 操作', diagnostics.databaseOperations],
	['PostgreSQL 平均延迟', `${diagnostics.databaseAverageLatencyMs.toFixed(1)}ms`],
	['PostgreSQL 最大延迟', `${diagnostics.databaseMaxLatencyMs.toFixed(1)}ms`],
