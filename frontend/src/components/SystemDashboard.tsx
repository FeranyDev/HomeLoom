...
function auditSummary(event: AuditEvent): string {
  if (event.route.includes('/properties/')) return event.outcome === 'succeeded' ? '已写入设备属性' : '设备属性写入失败'
  if (event.route.includes('/commands/')) return event.outcome === 'succeeded' ? '已执行设备命令' : '设备命令执行失败'
  const action = ({ restart: '重启', revoke: '撤销凭据', regenerate: '重新生成配对信息', 'factory-reset': '恢复出厂设置', import: '导入', scan: '扫描', test: '测试', post: '创建', put: '更新', patch: '更新', delete: '删除', 'matter.endpoint.allocate': '分配 Matter 端点', 'matter.endpoint.tombstone': '移除 Matter 端点', 'matter.endpoint.device-type-confirm': '确认 Matter 设备类型', 'matter.identity.clear': '清除 Matter 身份', 'matter.identity.delete': '删除 Matter 身份' } as Record<string, string>)[event.action] ?? event.action
  const resource = ({ provider: 'Provider', device: '设备', target: '目标桥接', 'mapping-profile': '映射配置', 'mapping-binding': '映射绑定', 'logical-device': '逻辑设备', 'matter-identity': 'Matter 身份', system: '系统' } as Record<string, string>)[event.resourceType] ?? event.resourceType
  return `${event.outcome === 'succeeded' ? '已完成' : '未完成'}：${action} ${resource}`
}

function isRoutineMatterStorageAudit(event: AuditEvent): boolean {
  return event.actor === 'matter-runtime' && event.method === 'IPC' && event.route === 'matter-runtime/storage' && ['matter.identity.write', 'matter.identity.read', 'matter.identity.list'].includes(event.action)
}
...
function RuntimeSettingsCard({ settings, onSave }: { settings: RuntimeSettings | null; onSave: (settings: RuntimeSettings) => Promise<void> }) {
  ...
  return <div className="queue-card settings-card"><div><span><HelpTooltip label="命令设置说明" content="这里仅设置命令超时和历史记录。写后回读请在设备的单属性映射中开启。">命令设置</HelpTooltip></span><strong>{settings?.commandTimeoutSeconds ?? seconds}s / {settings?.commandHistoryLimit ?? historyLimit} 条</strong></div><label>确认超时秒数<input aria-label="命令确认超时秒数" type="number" min="1" max="300" value={seconds} onChange={(event) => setSeconds(Number(event.target.value))} /></label><label>历史保留上限<input aria-label="命令历史保留上限" type="number" min="100" max="10000" value={historyLimit} onChange={(event) => setHistoryLimit(Number(event.target.value))} /></label><button className="primary" disabled={saving || !settings} onClick={() => void save()}>{saving ? '保存中…' : '保存并实时应用'}</button>{error && <small className="field-error" role="alert">{error}</small>}</div>
}
...
function DatabaseMaintenanceCard() {
	...
	return <div className="maintenance-card queue-card"><div><span><HelpTooltip label="备份与恢复说明" content="完整备份包含数据库、加密配置和主密钥。恢复前会验证备份，再在下次启动时替换当前数据。">备份与密钥</HelpTooltip></span><strong>管理员操作</strong></div><div className="maintenance-actions"><button className="primary" disabled={busy !== null} onClick={() => void backup()}>{busy === 'backup' ? '生成中…' : '下载完整备份'}</button><label className="file-picker">恢复压缩包<span>{restoreFile ? restoreFile.name : '选择 .zip 备份包'}</span><input aria-label="恢复压缩包" type="file" accept=".zip,application/zip" onChange={(event) => setRestoreFile(event.target.files?.[0] ?? null)} /></label><button className="is-danger" disabled={busy !== null || !restoreFile} onClick={() => void restore()}>{busy === 'restore' ? '校验中…' : '校验并暂存恢复'}</button></div><div className="maintenance-actions master-key-actions"><span><HelpTooltip label="主密钥说明" content="恢复整库需要重启。轮换保留旧密钥，用于恢复历史备份。">{keyStatus ? `主密钥 v${keyStatus.activeVersion} · 保留 v${keyStatus.retainedVersions.join('、v')}` : '请先读取主密钥状态'}</HelpTooltip></span><button disabled={busy !== null} onClick={() => void refreshKeyStatus()}>{busy === 'key-status' ? '读取中…' : '读取主密钥状态'}</button><button className="is-danger" disabled={busy !== null || !keyStatus} onClick={() => void rotateKey()}>{busy === 'key' ? '轮换中…' : keyStatus?.needsReencryption ? '恢复密钥轮换' : '轮换主密钥'}</button></div>{keyStatus?.needsReencryption && <small className="field-error" role="alert">检测到旧版本密文；请使用“恢复密钥轮换”完成批量重加密。</small>}{message && <small className="maintenance-message" role="status">{message}</small>}{error && <small className="field-error" role="alert">{error}</small>}</div>
}
...
export function SystemDashboard({ diagnostics, commands, auditEvents = [], settings, onSettingsSave }: { diagnostics: Diagnostics | null; commands: DeviceCommand[]; auditEvents?: AuditEvent[]; settings: RuntimeSettings | null; onSettingsSave: (settings: RuntimeSettings) => Promise<void> }) {
  ...
  const visibleAuditEvents = useMemo(() => auditEvents.filter((event) => !isRoutineMatterStorageAudit(event)), [auditEvents])
  const filteredAuditEvents = useMemo(() => {
    const normalizedAuditQuery = auditQuery.trim().toLowerCase()
    return visibleAuditEvents.filter((event) => !normalizedAuditQuery || `${event.correlationId} ${event.actor} ${event.action} ${event.resourceType} ${event.resourceId ?? ''} ${event.method} ${event.route} ${event.outcome} ${(event.details ?? []).map((item) => `${item.label} ${item.value}`).join(' ')}`.toLowerCase().includes(normalizedAuditQuery))
  }, [visibleAuditEvents, auditQuery])
  ...
  return <section className="system-dashboard"><div className="metric-grid">{metrics.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</div>
    <div className="queue-card"><div><span><HelpTooltip label="事件队列说明" content={`当前占用 ${queueRate}%。队列满时会丢弃并计数，不会阻塞设备来源。`}>事件队列</HelpTooltip></span><strong>{diagnostics.eventQueuePending} / {diagnostics.eventQueueCapacity}</strong></div><div className="queue-track"><span style={{ width: `${Math.min(queueRate, 100)}%` }} /></div></div>
    <RuntimeSettingsCard settings={settings} onSave={onSettingsSave} />
	<div className="artifact-card queue-card"><div><span><HelpTooltip label="支持资料说明" content="配置导出不包含桥 PIN、Setup URI 或本地路径；凭据会替换为星号。诊断包另含版本、运行指标和最近审计记录。">支持资料</HelpTooltip></span><strong><HelpTooltip label="分享资料说明" content="下载内容不会被浏览器缓存；分享前仍请检查设备名称等非凭据数据。">已自动脱敏</HelpTooltip></strong></div><div className="artifact-actions"><a className="ui-button" href="/api/v1/system/config-export" download>导出配置</a><a className="ui-button is-primary" href="/api/v1/system/diagnostic-bundle" download>下载诊断包</a></div></div>
	<DatabaseMaintenanceCard />
	<div className="subprocess-log-launcher queue-card"><div><span><HelpTooltip label="运行日志说明" content="主进程和子进程日志会集中采集并脱敏。日志实时推送，断线后自动补回近期记录。">实时运行日志</HelpTooltip></span><strong>HomeLoom · Camera Kernel · Matter</strong></div><button className="primary" type="button" onClick={() => setSubprocessLogsOpen(true)}>打开日志</button></div>
	{subprocessLogsOpen && <RuntimeLogDialog onClose={() => setSubprocessLogsOpen(false)} />}
	<div className="audit-section command-section"><div className="command-heading"><h3><HelpTooltip label="审计日志说明" content="最多保存 5000 条记录，页面会加载最近 200 条。">审计日志</HelpTooltip></h3></div><div className="command-filters"><input aria-label="搜索审计日志" value={auditQuery} onChange={(event) => setAuditQuery(event.target.value)} placeholder="搜索操作人、资源、参数、路由或 correlation ID" /><span>{filteredAuditEvents.length} / {visibleAuditEvents.length}</span></div>{visibleAuditEvents.length === 0 ? <div className="command-empty">还没有管理操作记录</div> : filteredAuditEvents.length === 0 ? <div className="command-empty">没有匹配的审计记录</div> : <><div className="audit-table"><div className="audit-row command-header"><span>发生了什么</span><span>操作人 / 结果</span><span>Correlation ID</span><span>时间</span></div>{auditPagination.pageItems.map((event) => <div className="audit-row" key={event.id}><span><b>{auditSummary(event)}</b><small>{event.resourceType}{event.resourceId ? ` · ${event.resourceId}` : ''}</small><small>{event.method} {event.route} · {event.action}</small>{(event.details ?? []).map((detail, index) => <small className="audit-detail" key={`${detail.label}-${index}`}><em>{detail.label}</em><code>{detail.value}</code></small>)}</span><span><small>操作人：{event.actor}</small><i className={`command-status is-${event.outcome === 'succeeded' ? 'confirmed' : 'rejected'}`}>{event.status}</i><small>{event.outcome === 'succeeded' ? '已完成' : '未完成'}</small></span><code title={event.correlationId}>{event.correlationId}</code><time>{new Date(event.createdAt).toLocaleString()}</time></div>)}</div><ListPagination label="审计日志" page={auditPagination.page} totalPages={auditPagination.totalPages} totalItems={auditPagination.totalItems} start={auditPagination.start} end={auditPagination.end} pageSize={auditPageSize} onPageChange={setAuditPage} onPageSizeChange={setAuditPageSize} /></>}</div>
