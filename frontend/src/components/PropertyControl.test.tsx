...
describe('PropertyControl', () => {
  it('writes typed boolean and number values', async () => {
    const writeBool = vi.fn().mockResolvedValue(undefined); const { unmount } = render(<PropertyControl definition={{ id: 'power', name: 'Power', type: 'bool', readable: true, writable: true, notifiable: true }} value={{ type: 'bool', bool: false }} onWrite={writeBool} />)
