...
export function CommandControl({ definition, onExecute }: { definition: CommandDefinition; onExecute: (parameters: Record<string, PropertyValue>, idempotencyKey: string) => Promise<void> }) {
  ...
  return <div className="command-control"><div><strong>{definition.name}</strong><code>{definition.id}</code><small>{definition.idempotent ? '可安全重放' : '禁止自动重试'}</small></div>{definition.parameters?.map((parameter) => <label key={parameter.id}><span>{parameter.name}{parameter.required ? ' *' : ''}</span>{parameter.type === 'bool' ? <select value={drafts[parameter.id]} onChange={(event) => setDrafts((current) => ({ ...current, [parameter.id]: event.target.value }))}><option value="false">false</option><option value="true">true</option></select> : <input type={parameter.type === 'number' ? 'number' : 'text'} value={drafts[parameter.id] ?? ''} onChange={(event) => setDrafts((current) => ({ ...current, [parameter.id]: event.target.value }))} />}</label>)}<button disabled={pending} onClick={() => void execute()}>{pending ? '执行中…' : '执行命令'}</button>{error && <small>{error}</small>}</div>
}
