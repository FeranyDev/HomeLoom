...
function CameraPreview({ deviceId }: { deviceId: string }) {
  ...
  return <section className="camera-preview" aria-label="摄像头实时预览">
    <div className="camera-preview-heading"><div><span>媒体诊断（MEDIA DIAGNOSTICS）</span><strong>摄像头实时画面</strong></div><button onClick={retry}>重新连接</button></div>
    ...
    <small>此预览由 HomeLoom 登录保护的本机代理提供，不会向浏览器暴露小米 Token 或发布器地址。</small>
  </section>
}
