...
describe('SystemDashboard', () => {
  ...
  it('shows queue health and command lifecycle', () => {
    ...
    expect(screen.getByText('数据库操作')).toBeInTheDocument(); expect(screen.getByText('数据库平均延迟')).toBeInTheDocument(); expect(screen.getByText('数据库最大延迟')).toBeInTheDocument(); expect(screen.getByText('Provider 事件时钟漂移')).toBeInTheDocument(); expect(screen.getByText('Provider 刷新旧快照')).toBeInTheDocument(); expect(screen.getByText('xiaomi/cloud-sensor')).toBeInTheDocument(); expect(screen.getByText('写后回读改为在设备的单属性映射中独立启用；这里仅配置命令超时与历史记录。')).toBeInTheDocument(); expect(screen.getByText('数据库持久化最近 5000 条 · 页面加载最近 200 条')).toBeInTheDocument(); expect(screen.queryByText(/PostgreSQL/)).not.toBeInTheDocument()
  })
