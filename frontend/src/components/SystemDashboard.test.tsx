...
describe('SystemDashboard', () => {
  ...
  it('shows queue health and command lifecycle', () => {
    ...
    expect(screen.getByText('数据库操作')).toBeInTheDocument(); expect(screen.getByText('数据库平均延迟')).toBeInTheDocument(); expect(screen.getByText('数据库最大延迟')).toBeInTheDocument(); expect(screen.getByText('Provider 事件时钟漂移')).toBeInTheDocument(); expect(screen.getByText('Provider 刷新旧快照')).toBeInTheDocument(); expect(screen.getByText('xiaomi/cloud-sensor')).toBeInTheDocument(); expect(screen.getByText('存储于数据库。超时仅影响之后创建的命令；降低历史上限会立即清理最旧的终态记录，不删除执行中的命令。')).toBeInTheDocument(); expect(screen.getByText('数据库持久化最近 5000 条 · 页面加载最近 200 条')).toBeInTheDocument(); expect(screen.queryByText(/PostgreSQL/)).not.toBeInTheDocument()
  })
