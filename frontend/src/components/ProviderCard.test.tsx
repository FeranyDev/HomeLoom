...
describe('ProviderCard simulation', () => {
  it('sends ephemeral availability and temperature changes', async () => {
    ...
    render(<ProviderCard provider={provider} devices={[{ schemaVersion: 1, id: 'temp-1', providerId: provider.id, name: '温度', type: 'temperature-sensor', online: true, endpoints: [], lastUpdateAt: '' }]} onEdit={() => {}} onDelete={() => {}} onRestart={onRestart} onSimulate={onSimulate} />)
  ...
  it('sends humidity, contact, and motion changes', async () => {
    ...
    const devices = [
      { schemaVersion: 1, id: 'humidity', providerId: provider.id, name: '湿度', type: 'humidity-sensor' as const, online: true, endpoints: [{ id: 'main', name: 'Main', type: 'sensor', capabilities: [{ id: 'humidity', type: 'humidity-sensor', properties: [{ definition: { id: 'current-humidity', name: '湿度', type: 'number' as const, readable: true, writable: false, notifiable: true }, value: { type: 'number' as const, number: 50 } }] }] }], lastUpdateAt: '' },
      { schemaVersion: 1, id: 'door', providerId: provider.id, name: '门', type: 'contact-sensor' as const, online: true, endpoints: [], lastUpdateAt: '' },
      { schemaVersion: 1, id: 'motion', providerId: provider.id, name: '活动', type: 'motion-sensor' as const, online: true, endpoints: [], lastUpdateAt: '' },
    ]
