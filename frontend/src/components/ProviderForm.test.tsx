...
describe('ProviderForm', () => {
  ...
  it('creates an independent camera provider without selecting an output target', async () => {
	...
	it('creates an empty Gree Provider and defers device setup to its manager', async () => {
		...
		await userEvent.hover(screen.getByRole('button', { name: '格力设备下一步说明' }))
		expect(screen.getByRole('tooltip')).toHaveTextContent('保存并运行后')
	...
	it('creates an empty network Provider and defers its device catalog to the shared manager', async () => {
		...
		await userEvent.hover(screen.getByRole('button', { name: '网络设备下一步说明' }))
		expect(screen.getByRole('tooltip')).toHaveTextContent('Provider 可先空目录运行')
	...
	it('moves Sonoff discovery into the persistent device manager', async () => {
		...
		await userEvent.hover(screen.getByRole('button', { name: '易微联下一步说明' }))
		expect(screen.getByRole('tooltip')).toHaveTextContent('加入清单并保存')
	...
	it('exposes MQTT client and server as top-level provider choices without a second mode selector', async () => {
		...
		expect(screen.getByText('MQTT 服务端')).toBeInTheDocument()
		await userEvent.hover(screen.getByRole('button', { name: 'MQTT 模式说明' }))
		expect(screen.getByRole('tooltip')).toHaveTextContent('内嵌 Broker')
