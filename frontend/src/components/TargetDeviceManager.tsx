...
export function TargetDeviceManager({ target, devices, onClose, onSave }: { target: Target; devices: Device[]; onClose: () => void; onSave: (input: TargetInput) => Promise<void> }) {
	...
	async function save() {
		...
		try { await onSave(inputFromTarget(target, items)) } catch (cause) { setError(cause instanceof Error ? cause.message : '保存消费端设备失败') } finally { setSaving(false) }
	}
	...
	return <section className="target-device-manager">
		...
		<div className="target-device-create"><label>来源统一设备（sourceDeviceId）<select aria-label="来源统一设备" value={sourceDeviceId} onChange={(event) => setSourceDeviceId(event.target.value)}><option value="">请选择</option>{availableSources.map((item) => <option key={item.id} value={item.id}>{item.name} · {deviceTypeLabel(item.type)} · {item.id}</option>)}</select></label><label>消费端设备标识（ID）<input value={id} placeholder="留空自动生成" onChange={(event) => setID(event.target.value)} /></label><label>显示名称（name）<input value={name} placeholder={source?.name ?? '消费端设备名称'} onChange={(event) => setName(event.target.value)} /></label><button disabled={!source} onClick={add}>＋ 添加消费端设备</button></div>
		...
		{mappingItem && mappingSource && <BindingManager device={mappingSource} initialStage="consumer" consumerOnly consumerId={consumerId} consumerLabel={`${mappingItem.name} · ${descriptor.consumerName} 属性映射`} targetId={target.id} consumerDeviceId={mappingItem.id} />}
