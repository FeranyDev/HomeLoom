...
export function CustomModelPropertyManager({ onChanged, deviceType, embedded = false, createRevision = 0 }: CustomModelPropertyManagerProps) {
  ...
  return <section className={`custom-model-manager ${embedded ? 'is-embedded' : ''}`}>
    <div className="profile-heading"><div><p className="eyebrow">统一模型自定义属性（CUSTOM UNIFIED PROPERTIES）</p><h3>{deviceType ? `${deviceTypeLabel(deviceType)} · 自定义属性` : '统一模型自定义属性'}</h3><p>自定义属性使用端点 / 能力 / 属性（Endpoint / Capability / Property）三级地址，完整定义类型、单位、范围和读 / 写 / 通知（R/W/N）权限，并保存到数据库。</p></div>{!embedded && <button onClick={() => edit()}>＋ 新建自定义属性</button>}</div>
