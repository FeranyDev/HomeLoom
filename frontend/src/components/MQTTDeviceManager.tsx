...
function configuredRoutes(config: Record<string, unknown>): MQTTRoute[] {
	...
	return config.devices.flatMap((value) => {
		...
		return [{ id, name: String(item.name ?? id), topicPrefix, protocol: 'homeloom-v1' as const, qos: Number(item.qos ?? 1), topics: { discovery: String(topics.discovery ?? defaults.discovery), availability: String(topics.availability ?? defaults.availability), state: String(topics.state ?? defaults.state), command: String(topics.command ?? defaults.command) } }]
	})
}

const blank = { id: '', name: '', topicPrefix: '', qos: 1, discovery: '', availability: '', state: '', command: '' }
...
export function MQTTDeviceManager({ provider, devices, onClose, onSave }: { provider: Provider; devices: Device[]; onClose: () => void; onSave: (input: ProviderInput, editing: boolean) => Promise<void> }) {
	...
	function edit(item: MQTTRoute) {
		...
		setEditingID(item.id); setError(null); setDraft({ id: item.id, name: item.name, topicPrefix: item.topicPrefix, qos: item.qos, discovery: item.topics.discovery === defaults.discovery ? '' : item.topics.discovery, availability: item.topics.availability === defaults.availability ? '' : item.topics.availability, state: item.topics.state === defaults.state ? '' : item.topics.state, command: item.topics.command === defaults.command ? '' : item.topics.command })
	}
	function stageRoute(event: React.FormEvent) {
		...
		const route: MQTTRoute = { id, name, topicPrefix: prefix, protocol: 'homeloom-v1', qos: Number(draft.qos), topics }
	...
	return <section className="mqtt-device-manager">
		...
		<div className="mqtt-route-workbench">
			<form onSubmit={stageRoute} className="mqtt-route-editor">
				<div><strong>{editingID ? `编辑 ${editingID} 草稿` : '添加设备路由草稿'}</strong><small>保存后订阅 retained Discovery，收到结构后才会发布设备。</small></div>
				...
				<label>设备名称<input aria-label="MQTT 设备名称" value={draft.name} onChange={(event) => update('name', event.target.value)} placeholder="客厅灯" /></label>
				...
				<div className="simulation-actions"><button type="submit">{editingID ? '更新草稿' : '加入草稿'}</button>{editingID && <button type="button" onClick={resetEditor}>取消编辑</button>}</div>
			</form>
