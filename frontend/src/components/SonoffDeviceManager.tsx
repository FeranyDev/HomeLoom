...
export function SonoffDeviceManager({ provider, devices, onClose, onSave }: {
	...
	return <section className="xiaomi-device-manager">
		<header><div><p className="eyebrow">SONOFF · EWELINK DEVICES</p><h3><HelpTooltip content="云目录、局域网扫描和已保存清单在这里合并；保存后设备才会稳定发布。" label="易微联设备说明">{provider.name} · 易微联设备</HelpTooltip></h3></div><button onClick={onClose}>返回 Provider</button></header>
		...
		<div className="xiaomi-device-binding"><div className="xiaomi-device-binding__heading"><div><strong>易微联设备目录</strong><small>已保存设备始终显示；云端或局域网短暂未发现不会自动移除。</small></div><span>{directory.length} 台</span></div>
			...
			<div className="xiaomi-mapped-summary"><strong><HelpTooltip content="保存后设备会以原内部 ID 保留；云端暂时为空、网络波动或服务重启都不会自动移除。" label="受管设备说明">已管理 {entries.length} 台设备</HelpTooltip></strong></div>
		</div>
