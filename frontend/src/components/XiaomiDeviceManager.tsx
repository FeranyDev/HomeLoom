...
export function XiaomiDeviceManager({ provider, onClose, onSave }: {
	...
	function addDevice(item: XiaomiHubDevice) {
		...
		replaceMappings([...mappings, { did: item.did, id, name: item.name || item.did, type, model: item.model ?? '', room: item.roomName ?? '', properties: requiredXiaomiProperties(type), actions: [] }])
	}
	...
	return <section className="xiaomi-device-manager">
		...
		<div className="xiaomi-device-binding">
			...
			{hubDevices.length === 0 ? <p className="xiaomi-device-binding__empty">连接就绪后读取设备目录，再选择需要作为来源发布的设备。</p> : <div className="xiaomi-hub-device-list">{hubDevices.map((item) => { const mapped = mappedDIDs.has(item.did); return <article key={item.did}><div><strong>{item.name || item.did}</strong><small>{item.roomName || '未分配房间'} · {item.model || item.specType || '型号未知'}</small><code>{item.did}</code></div><label>统一模型（deviceType）<select aria-label={`${item.name || item.did} 统一模型`} value={deviceTypes[item.did] ?? inferXiaomiDeviceType(item)} disabled={mapped} onChange={(event) => setDeviceTypes((current) => ({ ...current, [item.did]: event.target.value }))}>{xiaomiDeviceTypes.map(([value, label]) => <option value={value} key={value}>{label}（{value}）</option>)}</select></label>{mapped ? <button type="button" className="is-danger" onClick={() => removeDevice(item.did)}>移除映射</button> : <button type="button" onClick={() => addDevice(item)}>加入映射</button>}</article> })}</div>}
