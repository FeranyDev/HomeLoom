...
function xiaomiMappingDraft(providerType: string, item: XiaomiHubDevice, type: string, connectionMode: string, name?: string): Record<string, unknown> {
	...
	return {
		did: item.did, id, name: name?.trim() || item.name || item.did, type, model: item.model ?? '',
...
export function XiaomiDeviceManager(props: XiaomiDeviceManagerProps) {
	if (props.provider.type === 'network') return <NetworkDeviceManager provider={props.provider} devices={props.devices ?? []} onClose={props.onClose} onSave={props.onSave} />
	return <XiaomiDeviceManagerContent {...props} />
}

function XiaomiDeviceManagerContent({ provider, devices = [], locations = [], onClose, onSave, onMapping, onManageLocations }: XiaomiDeviceManagerProps) {
	...
	const [deviceNames, setDeviceNames] = useState<Record<string, string>>(() => Object.fromEntries(initialMappings.map((item) => [String(item.did ?? ''), String(item.name ?? '')]).filter(([did]) => did)))
	...
	async function discover() {
		...
		try {
			...
			setDeviceNames((current) => ({ ...Object.fromEntries(items.map((item) => [item.did, String(item.name || item.did)])), ...current }))
	...
	function addDevice(item: XiaomiHubDevice) {
		...
		const name = String(deviceNames[item.did] ?? item.name ?? item.did).trim() || item.did
		setLocationDrafts((current) => current[item.did] ? current : { ...current, [item.did]: { mode: 'source', homeId: '', roomId: '' } })
		replaceMappings([...mappings, xiaomiMappingDraft(provider.type, item, type, connectionMode, name)])
	}
	...
	function addUnmappedDrafts() {
		...
		for (const item of hubDevices) {
			...
			const draft = xiaomiMappingDraft(provider.type, item, type, connectionModes[item.did] ?? 'auto', deviceNames[item.did])
	...
	function updateDeviceName(did: string, value: string) {
		setDeviceNames((current) => ({ ...current, [did]: value }))
		if (mappedDIDs.has(did)) replaceMappings(mappings.map((item) => String(item.did ?? '') === did ? { ...item, name: value } : item))
	}
	...
	return <section className="xiaomi-device-manager">
		...
		<div className="xiaomi-device-binding">
			<div className="xiaomi-device-binding__heading"><div><strong>{central ? '中枢与官方云设备目录' : 'MIoT 云设备目录'}</strong><small>选择统一模型和设备级连接策略；已加入草稿的设备也可以直接调整。</small></div><span>{filteredHubDevices.length} / {hubDevices.length} 台</span>{hubDevices.length > 0 && <button type="button" disabled={saving} onClick={addUnmappedDrafts}>将未添加设备加入草稿</button>}</div>
