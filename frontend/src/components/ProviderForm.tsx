...
export function ProviderForm({ provider, initialType, onCancel, onSave, onTest }: { provider: Provider | null; initialType?: ProviderSelection | 'mqtt'; onCancel: () => void; onSave: (input: ProviderInput, editing: boolean) => Promise<void>; onTest?: (input: ProviderInput) => Promise<void> }) {
  ...
  const example = { latencyMs: 0, rejectWrites: false, devices: [{ id: 'demo-switch', name: '客厅开关', type: 'switch', online: true, power: false }, { id: 'demo-light', name: '客厅灯', type: 'lightbulb', online: true, power: true, brightness: 80, colorTemperature: 250, hue: 35, saturation: 45 }, { id: 'demo-outlet', name: '书房插座', type: 'outlet', online: true, power: false, inUse: false, currentPower: 0, energy: 1.25 }, { id: 'demo-temperature', name: '客厅温度', type: 'temperature-sensor', online: true, temperature: 23.6, batteryLevel: 91, lowBattery: false }, { id: 'demo-humidity', name: '客厅湿度', type: 'humidity-sensor', online: true, humidity: 56.2, batteryLevel: 90, lowBattery: false }, { id: 'demo-climate', name: '客厅温湿度', type: 'temperature-humidity-sensor', online: true, temperature: 23.6, humidity: 56.2, batteryLevel: 87, lowBattery: false }, { id: 'demo-contact', name: '入户门', type: 'contact-sensor', online: true, contact: false, batteryLevel: 88, lowBattery: false, tampered: false }, { id: 'demo-motion', name: '走廊活动', type: 'motion-sensor', online: true, motion: false, batteryLevel: 84, lowBattery: false, tampered: false }, { id: 'demo-fan', name: '卧室风扇', type: 'fan', online: true, active: false, speed: 35, mode: 'manual', swingMode: true, direction: 'clockwise', controlLock: false }, { id: 'demo-air', name: '客厅净化器', type: 'air-purifier', online: true, active: true, speed: 60, mode: 'auto', swingMode: false, controlLock: false, airQuality: 'good', pm25: 12, voc: 80, filterLife: 82, filterChange: false }, { id: 'demo-shade', name: '南窗帘', type: 'window-covering', online: true, position: 50, obstruction: false }, ...expandedVirtualExamples] }
	...
	const addNetworkDevice = () => {
		...
		updateNetworkDevices([...networkDevices, { id: `network-device-${suffix}`, name: '', host: '', probePort: 80, mac: '' }])
	}
	...
	async function testConnection() {
		...
		const networkConfiguration = <div className="wide xiaomi-connection-flow">
			<section className="xiaomi-connection-step"><div className="xiaomi-connection-step__heading"><span>01</span><div><strong>配置局域网设备监测</strong><small>HomeLoom 通过 TCP 端口探测设备可达性；设备配置 MAC 后可发送 Wake-on-LAN 魔术包，实际在线状态仍由后续探测确认。</small></div></div><div className="mqtt-config-grid">
				...
				<label>探测超时（probeTimeoutSeconds）<input aria-label="网络设备探测超时" type="number" min="1" max="120" value={Number(configObject.probeTimeoutSeconds ?? 3)} onChange={(event) => updateNetworkRuntime('probeTimeoutSeconds', Number(event.target.value))} /><small>TCP 建连最长等待时间，默认 3 秒。</small></label>
				<label>在线阈值（onlineThreshold）<input aria-label="网络设备在线阈值" type="number" min="1" max="100" value={Number(configObject.onlineThreshold ?? 1)} onChange={(event) => updateNetworkRuntime('onlineThreshold', Number(event.target.value))} /><small>连续成功达到阈值后才显示在线。</small></label>
				<label>离线阈值（offlineThreshold）<input aria-label="网络设备离线阈值" type="number" min="1" max="100" value={Number(configObject.offlineThreshold ?? 2)} onChange={(event) => updateNetworkRuntime('offlineThreshold', Number(event.target.value))} /><small>连续失败达到阈值后才显示离线。</small></label>
			...
			<section className="xiaomi-connection-step"><div className="xiaomi-connection-step__heading"><span>02</span><div><strong>监测设备</strong><small>每台设备需要稳定 ID、名称、Host 和 TCP 探测端口；MAC 留空时为仅监测设备，填写后可使用 Wake-on-LAN。</small></div></div>
				<div className="provider-device-list"><div className="command-heading"><h3>网络设备</h3><span>{networkDevices.length} 台</span></div>{networkDevices.length === 0 ? <p>还没有监测设备。添加至少一台设备后才能保存 Provider。</p> : networkDevices.map((device, index) => <section className="xiaomi-device-binding" key={index} aria-label={`网络设备 ${index + 1}`}><div className="xiaomi-device-binding__heading"><div><strong>{String(device.name || `网络设备 ${index + 1}`)}</strong><small>{String(device.host || '尚未设置 Host')}:{Number(device.probePort ?? 0) || '未设置端口'}{device.mac ? ' · Wake-on-LAN 已启用' : ' · 仅监测'}</small></div><button type="button" className="is-danger" aria-label={`移除网络设备 ${index + 1}`} onClick={() => removeNetworkDevice(index)}>移除</button></div><div className="mqtt-config-grid"><label>ID<input aria-label={`网络设备 ${index + 1} ID`} value={String(device.id ?? '')} onChange={(event) => updateNetworkDevice(index, 'id', event.target.value)} placeholder="living-room-pc" /></label><label>名称<input aria-label={`网络设备 ${index + 1} 名称`} value={String(device.name ?? '')} onChange={(event) => updateNetworkDevice(index, 'name', event.target.value)} placeholder="客厅电脑" /></label><label>Host<input aria-label={`网络设备 ${index + 1} Host`} value={String(device.host ?? '')} onChange={(event) => updateNetworkDevice(index, 'host', event.target.value)} placeholder="192.168.1.100" /></label><label>TCP 探测端口<input aria-label={`网络设备 ${index + 1} 探测端口`} type="number" min="1" max="65535" value={device.probePort === undefined ? '' : Number(device.probePort)} onChange={(event) => updateNetworkDevice(index, 'probePort', numericOverride(event.target.value))} placeholder="例如 445、443 或 3389" /></label><label className="wide">MAC（用于 Wake-on-LAN，可选）<input aria-label={`网络设备 ${index + 1} MAC`} value={String(device.mac ?? '')} onChange={(event) => updateNetworkDevice(index, 'mac', event.target.value)} placeholder="AA:BB:CC:DD:EE:FF" /><small>发送 Wake 命令后不会直接改为在线，仍会由后续 TCP 探测确认。</small></label></div><details><summary>单项高级覆盖（可选）</summary><div className="mqtt-tls-grid"><label>探测间隔（秒）<input aria-label={`网络设备 ${index + 1} 探测间隔覆盖`} type="number" min="1" max="3600" value={device.probeIntervalSeconds === undefined ? '' : Number(device.probeIntervalSeconds)} onChange={(event) => updateNetworkDevice(index, 'probeIntervalSeconds', numericOverride(event.target.value))} placeholder="继承全局设置" /></label><label>探测超时（秒）<input aria-label={`网络设备 ${index + 1} 探测超时覆盖`} type="number" min="1" max="120" value={device.probeTimeoutSeconds === undefined ? '' : Number(device.probeTimeoutSeconds)} onChange={(event) => updateNetworkDevice(index, 'probeTimeoutSeconds', numericOverride(event.target.value))} placeholder="继承全局设置" /></label><label>在线阈值<input aria-label={`网络设备 ${index + 1} 在线阈值覆盖`} type="number" min="1" max="100" value={device.onlineThreshold === undefined ? '' : Number(device.onlineThreshold)} onChange={(event) => updateNetworkDevice(index, 'onlineThreshold', numericOverride(event.target.value))} placeholder="继承全局设置" /></label><label>离线阈值<input aria-label={`网络设备 ${index + 1} 离线阈值覆盖`} type="number" min="1" max="100" value={device.offlineThreshold === undefined ? '' : Number(device.offlineThreshold)} onChange={(event) => updateNetworkDevice(index, 'offlineThreshold', numericOverride(event.target.value))} placeholder="继承全局设置" /></label><label>WOL 广播地址<input aria-label={`网络设备 ${index + 1} WOL 广播地址覆盖`} value={String(device.wolBroadcastAddress ?? '')} onChange={(event) => updateNetworkDevice(index, 'wolBroadcastAddress', event.target.value || undefined)} placeholder="继承全局设置" /></label><label>WOL 端口<input aria-label={`网络设备 ${index + 1} WOL 端口覆盖`} type="number" min="1" max="65535" value={device.wolPort === undefined ? '' : Number(device.wolPort)} onChange={(event) => updateNetworkDevice(index, 'wolPort', numericOverride(event.target.value))} placeholder="继承全局设置" /></label><label className="wide">WOL 网络接口<input aria-label={`网络设备 ${index + 1} WOL 网络接口覆盖`} value={String(device.wolInterface ?? '')} onChange={(event) => updateNetworkDevice(index, 'wolInterface', event.target.value || undefined)} placeholder="继承全局设置" /></label></div></details></section>)}</div>
				<button type="button" className="example-button" onClick={addNetworkDevice}>添加网络设备</button>
			</section>
			<div className="config-note"><span>设备状态与唤醒</span><strong>TCP 可达即在线；Wake 操作不会乐观改写状态</strong><p>可在每台设备的高级区域覆盖探测和 WOL 设置；未填写的项目继承本页的全局配置。</p></div>
