  ...
  return <article className="profile-transform-card">
    ...
    {transform.type === 'enum' && (() => {
      ...
      return <div className="profile-enum-map">
        ...
        {values.map(([source, target], row) => <div key={row}><input aria-label={`第 ${index + 1} 步来源值 ${row + 1}`} value={source} onChange={(event) => { const next = Object.fromEntries(values.map(([key, value], item) => item === row ? [event.target.value, value] : [key, value])); updateValues(next) }} /><span>→</span><input aria-label={`第 ${index + 1} 步目标值 ${row + 1}`} value={target} onChange={(event) => { const next = Object.fromEntries(values.map(([key, value], item) => item === row ? [key, event.target.value] : [key, value])); updateValues(next) }} /><button aria-label={`删除第 ${index + 1} 步枚举行 ${row + 1}`} onClick={() => updateValues(Object.fromEntries(values.filter((_, item) => item !== row)))}>×</button></div>)}
