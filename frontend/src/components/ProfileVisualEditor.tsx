...
const transformGroups: { label: string; types: MappingTransformType[] }[] = [
  { label: '数值 ⇄ 枚举', types: ['range-enum', 'enum-number'] },
...
function defaultTransform(type: MappingTransformType): MappingTransform {
  ...
  if (type === 'range-enum' || type === 'enum-number') return { type, bands: [{ max: 18, value: 'cold', reverse: 10 }, { max: 28, value: 'comfortable', reverse: 24 }, { value: 'hot', reverse: 32 }] }
  if (type === 'threshold' || type === 'bool-number') return { type, threshold: 20, operator: 'gte', trueNumber: 25, falseNumber: 10 }
...
function transformHelp(type: MappingTransformType): string {
  switch (type) {
    case 'clamp': return '仅支持正向预览，不能用于要求可逆的设备属性映射。'
    case 'parse-number': return '将数字文本转为数值；反向生成数字文本。'
    case 'number-string': return '将数值转为文本；反向解析并校验数值。'
    case 'invert': return '交换 true 与 false，无需设置参数。'
    case 'reciprocal': return '计算输入值的倒数，输入不能为 0。'
    case 'int-number': return '将整数转为数值；反向仅接受安全范围内的整数。'
    default: return '支持正向转换与反向控制。'
  }
}
...
function supported(type: MappingTransformType, input: ValueType): boolean {
  ...
  if (type === 'bool-number') return input === 'bool'
...
function localErrors(profile: MappingProfile): string[] {
  const errors: string[] = []
  if (!/^[a-z0-9](?:[a-z0-9._-]{0,62}[a-z0-9])?$/.test(profile.identifier ?? '')) errors.push('标识（identifier）需为 1–64 位小写字母、数字、点、下划线或连字符，且首尾为字母或数字。')
  ...
  profile.transforms.forEach((transform, index) => {
    ...
    if (transform.type === 'enum') {
      ...
      for (const [target, source] of Object.entries(reverseValues)) {
        if (!target) errors.push(`第 ${index + 1} 步反向控制别名不能为空。`)
        else if (!(transform.values ?? {})[source]) errors.push(`第 ${index + 1} 步反向控制值“${target}”必须指向已有来源值。`)
        else if (sourcesByTarget.has(target) && !sourcesByTarget.get(target)?.includes(source)) errors.push(`第 ${index + 1} 步目标值“${target}”的反向代表值必须是映射到它的来源值之一。`)
      }
    }
    ...
    if (transform.type === 'threshold' || transform.type === 'bool-number') {
...
function editableProfile(initialProfile: MappingProfile): MappingProfile {
  // Older browser drafts and API responses used id as the display identifier.
  // Keep them editable while new profiles leave id empty for server assignment.
  return initialProfile.identifier ? initialProfile : { ...initialProfile, identifier: initialProfile.id }
}
...
function TransformEditor({ transform, index, count, onChange, onMove, onRemove }: {
  ...
  return <article className="profile-transform-card">
    <header><span>{String(index + 1).padStart(2, '0')}</span><div><strong><HelpTooltip label={`第 ${index + 1} 步转换说明`} content={transformHelp(transform.type)}>{transformTypeLabel(transform.type)}</HelpTooltip></strong></div><div><button aria-label={`上移第 ${index + 1} 步`} disabled={index === 0} onClick={() => onMove(-1)}>↑</button><button aria-label={`下移第 ${index + 1} 步`} disabled={index === count - 1} onClick={() => onMove(1)}>↓</button><button aria-label={`删除第 ${index + 1} 步`} onClick={onRemove}>×</button></div></header>
    {transform.type === 'scale' && <div className="profile-transform-fields"><label>缩放系数<input aria-label={`第 ${index + 1} 步缩放系数`} type="number" step="any" value={transform.factor ?? 1} onChange={(event) => onChange({ ...transform, factor: Number(event.target.value) })} /></label><label>偏移量<input aria-label={`第 ${index + 1} 步偏移量`} type="number" step="any" value={transform.offset ?? 0} onChange={(event) => onChange({ ...transform, offset: Number(event.target.value) })} /></label></div>}
    {transform.type === 'clamp' && <div className="profile-transform-fields"><label>最小值<input aria-label={`第 ${index + 1} 步最小值`} type="number" step="any" value={transform.min ?? ''} onChange={(event) => onChange({ ...transform, min: event.target.value === '' ? undefined : Number(event.target.value) })} /></label><label>最大值<input aria-label={`第 ${index + 1} 步最大值`} type="number" step="any" value={transform.max ?? ''} onChange={(event) => onChange({ ...transform, max: event.target.value === '' ? undefined : Number(event.target.value) })} /></label></div>}
    {transform.type === 'unit' && <label className="profile-unit-route">单位转换<select aria-label={`第 ${index + 1} 步单位路径`} value={`${transform.fromUnit}:${transform.toUnit}`} onChange={(event) => { const [fromUnit, toUnit] = event.target.value.split(':'); onChange({ type: 'unit', fromUnit, toUnit }) }}>{unitRoutes.map(([from, to]) => <option key={`${from}:${to}`} value={`${from}:${to}`}>{from} → {to}</option>)}</select></label>}
    {transform.type === 'enum' && (() => {
      const reverseValues = transform.reverseValues ?? {}
      const sourcesByTarget = new Map<string, string[]>()
      for (const [source, target] of values) {
      ...
      const aliases = Object.entries(reverseValues).filter(([target]) => !sourcesByTarget.has(target))
      const updateValues = (nextValues: Record<string, string>) => {
        ...
        for (const [target, sources] of nextSourcesByTarget) {
          ...
          if (sources.length > 1) {
			...
			// Preserve write-only aliases such as cool -> auto. They are not
			// forward targets and therefore must not disappear when the canonical
			// forward map is edited.
			for (const [target, source] of Object.entries(reverseValues)) {
				if (!nextSourcesByTarget.has(target) && Object.hasOwn(nextValues, source)) nextReverse[target] = source
			}
        onChange({ ...transform, values: nextValues, reverseValues: Object.keys(nextReverse).length ? nextReverse : undefined })
      }
		const updateAliases = (nextAliases: Array<[string, string]>) => {
			const nextReverse: Record<string, string> = {}
			for (const [target, source] of Object.entries(reverseValues)) {
				if (sourcesByTarget.has(target)) nextReverse[target] = source
			}
			for (const [target, source] of nextAliases) {
				if (!sourcesByTarget.has(target)) nextReverse[target] = source
			}
			onChange({ ...transform, reverseValues: Object.keys(nextReverse).length ? nextReverse : undefined })
		}
      return <div className="profile-enum-map">
        <div className="profile-enum-head"><span>来源值</span><span>目标值</span><span /></div>
        {values.map(([source, target], row) => <div className="profile-enum-row" key={row}><input aria-label={`第 ${index + 1} 步来源值 ${row + 1}`} value={source} onChange={(event) => { const next = Object.fromEntries(values.map(([key, value], item) => item === row ? [event.target.value, value] : [key, value])); updateValues(next) }} /><span>→</span><input aria-label={`第 ${index + 1} 步目标值 ${row + 1}`} value={target} onChange={(event) => { const next = Object.fromEntries(values.map(([key, value], item) => item === row ? [key, event.target.value] : [key, value])); updateValues(next) }} /><button aria-label={`删除第 ${index + 1} 步枚举行 ${row + 1}`} onClick={() => updateValues(Object.fromEntries(values.filter((_, item) => item !== row)))}>×</button></div>)}
        ...
        {foldedTargets.length > 0 && <div className="profile-enum-reverse">
          <div className="profile-enum-head"><span>折叠目标（target）</span><HelpTooltip label={`第 ${index + 1} 步反向代表值说明`} content="多个来源对应同一目标时，反向控制写回此处选定的来源值。">反向代表值</HelpTooltip><span /></div>
          {foldedTargets.map(([target, sources]) => <div className="profile-enum-row" key={target}>
            <code>{target}</code>
            <span>←</span>
        ...
        <div className="profile-enum-reverse profile-enum-aliases" aria-label="反向控制别名">
          <div className="profile-enum-section-heading">
            <strong><HelpTooltip label={`第 ${index + 1} 步反向别名说明`} content="每行左侧的控制值写回右侧来源值，仅影响反向控制，不改变正向状态。">反向控制别名</HelpTooltip></strong>

          </div>
          <div className="profile-enum-head profile-enum-alias-head"><span>设备控制值 <small>目标别名（target）</small></span><span>实际写回值 <small>来源值（source）</small></span><span /></div>
          {aliases.map(([target, source], row) => <div className="profile-enum-alias-row" key={row}>
            <input aria-label={`第 ${index + 1} 步反向别名 ${row + 1}`} value={target} onChange={(event) => updateAliases(aliases.map(([currentTarget, currentSource], item) => item === row ? [event.target.value, currentSource] : [currentTarget, currentSource]))} />
            <span>→</span>
            <select aria-label={`第 ${index + 1} 步反向别名来源 ${row + 1}`} value={source} onChange={(event) => updateAliases(aliases.map(([currentTarget, currentSource], item) => item === row ? [currentTarget, event.target.value] : [currentTarget, currentSource]))}>
              {values.map(([candidate]) => <option key={candidate} value={candidate}>{candidate}</option>)}
            </select>
            <button aria-label={`删除第 ${index + 1} 步反向别名 ${row + 1}`} onClick={() => updateAliases(aliases.filter((_, item) => item !== row))}>×</button>
          </div>)}
          <button disabled={values.length === 0} onClick={() => updateAliases([...aliases, [`alias-${aliases.length + 1}`, values[0]?.[0] ?? '']])}>＋ 添加反向控制别名</button>

        </div>
      </div>
    })()}
    {(transform.type === 'range-enum' || transform.type === 'enum-number') && <div className="profile-range-bands"><div className="profile-range-head"><HelpTooltip label={`第 ${index + 1} 步分段说明`} content="数值不超过上限时匹配该枚举；反向使用代表数值。最后一段上限留空。">数值上限</HelpTooltip><span>枚举值（value）</span><span>代表数值（reverse）</span><span /></div>{bands.map((band, row) => <div key={row}><input aria-label={`第 ${index + 1} 步分段上限 ${row + 1}`} type="number" step="any" placeholder={row === bands.length - 1 ? '+∞（留空）' : undefined} value={band.max ?? ''} onChange={(event) => onChange({ ...transform, bands: bands.map((item, itemIndex) => itemIndex === row ? { ...item, max: event.target.value === '' ? undefined : Number(event.target.value) } : item) })} /><input aria-label={`第 ${index + 1} 步分段枚举 ${row + 1}`} value={band.value} onChange={(event) => onChange({ ...transform, bands: bands.map((item, itemIndex) => itemIndex === row ? { ...item, value: event.target.value } : item) })} /><input aria-label={`第 ${index + 1} 步分段反向值 ${row + 1}`} type="number" step="any" value={band.reverse} onChange={(event) => onChange({ ...transform, bands: bands.map((item, itemIndex) => itemIndex === row ? { ...item, reverse: Number(event.target.value) } : item) })} /><button aria-label={`删除第 ${index + 1} 步分段 ${row + 1}`} onClick={() => onChange({ ...transform, bands: bands.filter((_, itemIndex) => itemIndex !== row) })}>×</button></div>)}<button onClick={() => { const fallback = bands.at(-1) ?? { value: 'high', reverse: 100 }; const previousMax = bands.length > 1 ? bands[bands.length - 2].max ?? 0 : 0; const max = previousMax + 10; onChange({ ...transform, bands: [...bands.slice(0, -1), { max, value: `band-${bands.length}`, reverse: previousMax + 5 }, fallback] }) }}>＋ 在最终分段前添加</button></div>}
    {(transform.type === 'threshold' || transform.type === 'bool-number') && <div className="profile-transform-fields profile-threshold-fields"><label>比较方式<select aria-label={`第 ${index + 1} 步比较方式`} value={transform.operator ?? 'gte'} onChange={(event) => onChange({ ...transform, operator: event.target.value as MappingTransform['operator'] })}><option value="gte">大于等于（≥）</option><option value="gt">大于（&gt;）</option><option value="lte">小于等于（≤）</option><option value="lt">小于（&lt;）</option></select></label><label>阈值<input aria-label={`第 ${index + 1} 步阈值`} type="number" step="any" value={transform.threshold ?? ''} onChange={(event) => onChange({ ...transform, threshold: Number(event.target.value) })} /></label><label>true 代表值<input aria-label={`第 ${index + 1} 步 true 反向值`} type="number" step="any" value={transform.trueNumber ?? ''} onChange={(event) => onChange({ ...transform, trueNumber: Number(event.target.value) })} /></label><label>false 代表值<input aria-label={`第 ${index + 1} 步 false 反向值`} type="number" step="any" value={transform.falseNumber ?? ''} onChange={(event) => onChange({ ...transform, falseNumber: Number(event.target.value) })} /></label></div>}
    ...
    {transform.type === 'map-range' && <div className="profile-transform-fields profile-range-fields"><label>输入最小值<input aria-label={`第 ${index + 1} 步输入最小值`} type="number" step="any" value={transform.inputMin ?? ''} onChange={(event) => onChange({ ...transform, inputMin: Number(event.target.value) })} /></label><label>输入最大值<input aria-label={`第 ${index + 1} 步输入最大值`} type="number" step="any" value={transform.inputMax ?? ''} onChange={(event) => onChange({ ...transform, inputMax: Number(event.target.value) })} /></label><label>输出最小值<input aria-label={`第 ${index + 1} 步输出最小值`} type="number" step="any" value={transform.outputMin ?? ''} onChange={(event) => onChange({ ...transform, outputMin: Number(event.target.value) })} /></label><label>输出最大值<input aria-label={`第 ${index + 1} 步输出最大值`} type="number" step="any" value={transform.outputMax ?? ''} onChange={(event) => onChange({ ...transform, outputMax: Number(event.target.value) })} /></label></div>}
    {transform.type === 'round' && <label className="profile-unit-route">取整方式<select aria-label={`第 ${index + 1} 步取整方式`} value={transform.mode ?? 'nearest'} onChange={(event) => onChange({ ...transform, mode: event.target.value as MappingTransform['mode'] })}><option value="nearest">四舍五入（nearest）</option><option value="floor">向下取整（floor）</option><option value="ceil">向上取整（ceil）</option></select></label>}
...
export function ProfileVisualEditor({ initialProfile, editing, saving, onClose, onSave, runPreview = previewMapping }: {
  ...
  const [document, setDocument] = useState(() => JSON.stringify(editableProfile(initialProfile), null, 2))
  ...
  return <section className="profile-editor profile-visual-editor" aria-label={editing ? `编辑 ${initialProfile.identifier ?? initialProfile.id}` : '新建 Profile'}>
    <header className="profile-editor-heading"><div><h3>{editing ? `编辑 ${initialProfile.identifier ?? initialProfile.id}` : '创建转换配置'}</h3></div><button aria-label="关闭 Profile 编辑器" onClick={onClose}>×</button></header>
    ...
    {editorMode === 'json' ? <div className="profile-json-editor"><textarea aria-label="Profile JSON" rows={22} value={document} onChange={(event) => setDocument(event.target.value)} spellCheck={false} />{documentError && <p className="field-error" role="alert">{documentError}</p>}<HelpTooltip label="JSON 编辑说明" content="可复制或修改完整规则；切回可视化时会解析当前内容。">JSON 编辑</HelpTooltip></div> : <>
      <section className="profile-definition-card"><header><span>01</span><div><strong><HelpTooltip label="转换接口说明" content="配置映射时，会按用途和数据类型筛选可用规则。">基本设置</HelpTooltip></strong></div></header><div className="profile-definition-grid"><label><HelpTooltip label="规则标识说明" content="用于搜索和导入导出，可随时修改。">标识</HelpTooltip><input aria-label="Profile 标识" value={profile.identifier ?? ''} onChange={(event) => setProfile({ ...profile, identifier: event.target.value })} /></label>{editing && <div className="profile-derived-output"><HelpTooltip label="规则 ID 说明" content="映射始终关联此 ID，修改标识不会影响已有映射。">规则 ID</HelpTooltip><code>{profile.id}</code></div>}<label>用途<select aria-label="Profile 用途" value={profile.kind} onChange={(event) => setProfile({ ...profile, kind: event.target.value as MappingProfile['kind'] })}><option value="provider">{profileKindLabel('provider')}</option><option value="capability">{profileKindLabel('capability')}</option><option value="target">{profileKindLabel('target')}</option></select></label><label>版本<input aria-label="Profile 版本" type="number" min="1" value={profile.version} onChange={(event) => setProfile({ ...profile, version: Number(event.target.value) })} /></label><label>输入类型<select aria-label="Profile 输入类型" value={profile.inputType} onChange={(event) => changeInputType(event.target.value as ValueType)}>{valueTypes.map((type) => <option key={type} value={type}>{valueTypeLabel(type)}</option>)}</select></label><div className="profile-derived-output"><HelpTooltip label="输出类型说明" content="根据转换步骤自动计算。">输出类型</HelpTooltip><strong>{valueTypeLabel(profile.outputType)}</strong></div><div className="profile-default-field"><label><input aria-label="启用缺失值默认回退" type="checkbox" checked={profile.default !== undefined} onChange={(event) => setProfile({ ...profile, default: event.target.checked ? propertyValue(profile.inputType, rawDefault(profile.inputType)) : undefined })} /><HelpTooltip label="默认值说明" content="来源值缺失时，以默认值参与转换。">使用默认值</HelpTooltip></label>{profile.default && <ValueInput label={`默认输入值 · ${valueTypeLabel(profile.inputType)}`} type={profile.inputType} value={valueText(profile.default)} onChange={(value) => setProfile({ ...profile, default: propertyValue(profile.inputType, value) })} />}</div></div></section>
      <section className="profile-pipeline-builder"><header><span>02</span><div><strong><HelpTooltip label="转换顺序说明" content="正向按步骤顺序执行，反向按相反顺序执行。">转换步骤</HelpTooltip></strong></div></header><div className="profile-pipeline-summary"><div><span>输入（input）</span><strong>{valueTypeLabel(profile.inputType)}</strong></div><i>→</i>{profile.transforms.length ? profile.transforms.map((transform, index) => <div key={`${transform.type}-${index}`}><span>步骤 {index + 1}</span><strong>{transformTypeLabel(transform.type)}</strong></div>) : <div className="is-identity"><span>无步骤</span><strong>恒等转换（identity）</strong></div>}<i>→</i><div><span>输出（output）</span><strong>{valueTypeLabel(profile.outputType)}</strong></div></div><div className="profile-transform-list">{profile.transforms.map((transform, index) => <TransformEditor key={`${transform.type}-${index}`} transform={transform} index={index} count={profile.transforms.length} onChange={(value) => replaceTransforms(profile.transforms.map((item, itemIndex) => itemIndex === index ? value : item))} onMove={(offset) => { const next = [...profile.transforms]; const [item] = next.splice(index, 1); next.splice(index + offset, 0, item); replaceTransforms(next) }} onRemove={() => replaceTransforms(profile.transforms.filter((_, itemIndex) => itemIndex !== index))} />)}</div><div className="profile-transform-picker"><span>添加转换步骤</span><div className="profile-transform-groups">{transformGroups.map((group) => <section key={group.label}><strong>{group.label}</strong><div>{group.types.map((type) => { const available = supported(type, profile.outputType); return <button key={type} disabled={!available} title={available ? '' : `不支持 ${valueTypeLabel(profile.outputType)}`} onClick={() => replaceTransforms([...profile.transforms, defaultTransform(type)])}>＋ {transformTypeLabel(type)}</button> })}</div></section>)}</div></div></section>
      <section className="profile-preview-card"><header><span>03</span><div><strong><HelpTooltip label="样本验证说明" content="仅验证当前草稿，不保存或控制真实设备。">样本验证</HelpTooltip></strong></div></header><div className="profile-preview-layout"><form onSubmit={(event) => { event.preventDefault(); void preview() }}><div className="profile-direction-toggle" role="group" aria-label="预览方向"><button type="button" className={direction === 'forward' ? 'is-active' : ''} onClick={() => { setDirection('forward'); setPreviewInput(rawDefault(profile.inputType)); setPreviewResult(null) }}>正向（forward）</button><button type="button" className={direction === 'reverse' ? 'is-active' : ''} disabled={!reverseAvailable} title={reverseAvailable ? '' : '范围裁剪无法反向执行'} onClick={() => { setDirection('reverse'); setPreviewInput(rawDefault(profile.outputType)); setPreviewResult(null) }}>反向（reverse）</button></div><ValueInput label={`样本输入 · ${valueTypeLabel(activeInputType)}`} type={activeInputType} value={previewInput} onChange={setPreviewInput} /><button className="add-button" disabled={previewing || errors.length > 0}>{previewing ? '计算中…' : '运行当前草稿'}</button>{!reverseAvailable && <small>当前包含范围裁剪（clamp），仅可验证正向流程。</small>}{previewError && <p className="field-error" role="alert">{previewError}</p>}</form><div className="profile-preview-result" aria-live="polite">{previewResult ? <><span>最终输出 · {valueTypeLabel(previewResult.value.type)}</span><strong>{valueText(previewResult.value)}</strong><ol>{previewResult.steps.map((step) => <li key={`${step.index}-${step.transform}`}><span>{step.index + 1}. {transformTypeLabel(step.transform as MappingTransformType)}</span><code>{step.input ? valueText(step.input) : 'null'} → {valueText(step.output)}</code></li>)}</ol></> : <div className="empty-state">暂无验证结果</div>}</div></div></section>
